...
class Pay07430091 extends PayAbstract
{
    /**
     * 到局方鉴权，只有包月产品才能鉴权。
     * @param
    ...
    public function authentication($param = null)
    {
        ...
        if ($respond["RST_CODE"] == "0" || $respond["RST_CODE"] == "2") {
            ...
            if ($desc && preg_match("/($desc)/", "免费周期鉴权通过")) {
                ...
                LogUtils::info("Pay07430091::authentication ---> is free experience!!!");
            } else {
                $isVip = 1;
            }
    ...
    private function queryUserOrderDetail($userAccount, $videoInfo)
    {
        ...
        $infoParam["PicUrl"] = RESOURCES_URL . $videoInfo->imageUrl;
    ...
    {
        LogUtils::info("Pay[07430091] --> direct go pay!!!");
    ...
    {
        LogUtils::info("payCallback[07430091] ---> _GET: " . json_encode($_GET));
        ...
        if ($RST['RST_CODE'] == "0" || $RST['RST_CODE'] == "2") {
            ...
            LogUtils::info("payCallback[07430091] pay success!!!!!!!!");
        } else {
        ...
        if (($RST['RST_CODE'] == "0" || $RST['RST_CODE'] == "2") && $isPlaying == "1" && $lmreason == 0) {
            ...
            LogUtils::info("payCallback[07430091] will go page =>: " . $goPlayUrl);
            ...
            LogUtils::info("payCallback[07430091] will go page =>: " . $returnUrl);
            $arr["jumpUrl"] = $returnUrl;
        }
        $jumpType = $arr["jumpType"];
        $jumpUrl = $arr["jumpUrl"];
        LogUtils::info("payCallback[07430091] ---> jumpUrl: " . $jumpUrl . "---->jumpType" . $jumpType);
