...
class Pay360092 extends PayAbstract
{
    /**
     * 用户到局方鉴权
     * @param $param
    ...
    public function authentication($param = null){
        ...
        if ($respond[1][0] == 0) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($productID == PRODUCT_ID_10 || $productID == PRODUCT_ID_15) {
                    // 判断失效时间
    ...
    public function buildPayUrl($payInfo = null){
        ...
        if ($payInfo->product_id == 1) { // 包月
            $productInfo->productId = PRODUCT_ID_10;        // 使用包月产品
            $productInfo->price = 1000;                     // 价格
            $productInfo->PurchaseType = 0;                 // 包月自动续费
            $productInfo->orderDescription = "整包订购，自动续包月。";
        } else { // 包30天
            $productInfo->productId = PRODUCT_ID_15;        // 使用包月产品
            $productInfo->price = 1500;                     // 价格
            $productInfo->PurchaseType = 0;                 // 包月自动续费
            $productInfo->orderDescription = "订购即生效，有效期内可无限次使用。";
        }
        ...
        // 得到EPG缓存信息
        $epgInfoMap = MasterManager::getEPGInfoMap();
        LogUtils::info("Pay360092::buildPayUrl() ---> epgInfoMap: " . json_encode($epgInfoMap));
    ...
    public function directToPay($orderInfo = null){
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay360092::directToPay() ---> orderItems is empty");
            IntentManager::back();
            exit();
        }
        // 直接订购，使用第一个订购项（包月订购项）。
        ...
        if ($tradeInfo->result != 0) {
            // 生成订购单号失败
            LogUtils::info("Pay360092::directToPay() ---> 拉取订单失败:" . $tradeInfo->result);
            IntentManager::back();
            exit();
        }
        // 鉴权失败说明用户可订购， 构建订购地址
    ...
    private function _buildPayUrl($userInfo, $payInfo, $productInfo)
    {
        ...
        LogUtils::info("Pay360092::_buildPayUrl() ---> infoParam: " . json_encode($infoParam));
        ...
        // 根据所选择的商品类型 -- 订购用途说明
        $payParam["orderDescription"] = rawurlencode(mb_convert_encoding($productInfo->orderDescription, 'GBK', 'utf-8'));
        ...
        LogUtils::info("Pay360092::_buildPayUrl() ---> payUrl: " . $payUrl);
        return $payUrl;
    }
    ...
    private function _buildPayCallbackUrl($param = null)
    {
        ...
        LogUtils::info("Pay360092::_buildPayCallbackUrl()  payBackUrl" . $url);
        return $url;
    }
    ...
    public function webPagePay($userInfo) {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay360092::webPagePay ---> pay orderItem is empty");
            return $payUrl;
        }
        ...
        LogUtils::info("Pay360092::webPagePay pay ---> tradeInfo: " . json_encode($tradeInfo));
        if ($tradeInfo->result == 0) {
            // 订购的产品信息
            $productInfo = new \stdClass();
            $productInfo->productId = PRODUCT_ID_10;        // 使用包月产品
