...
class Pay210092 extends PayAbstract
{
    /**
     * 用户到局方鉴权
     * @param $param
    ...
    public function authentication($param = null)
    {
        ...
        if ($respond[1][0] == 0) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($productID == PRODUCT_ID_18 || $productID == PRODUCT_ID_20
                    || $productID == PRODUCT_ID_27 || $productID == PRODUCT_ID_30) {
                    // 判断失效时间
    ...
    public function buildPayUrl($payInfo = null)
    {
        ...
        // 得到EPG缓存信息
        $epgInfoMap = MasterManager::getEPGInfoMap();
        LogUtils::info("Pay210092::buildPayUrl() ---> epgInfoMap: " . json_encode($epgInfoMap));
    ...
    public function directToPay($orderInfo = null)
    {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay210092::directToPay() ---> orderItems is empty");
            IntentManager::back();
            exit();
        }
        // 直接订购，使用第一个订购项（包月订购项）。
        ...
        if ($tradeInfo->result != 0) {
            // 生成订购单号失败
            LogUtils::info("Pay210092::directToPay() ---> 拉取订单失败:" . $tradeInfo->result);
            IntentManager::back();
            exit();
        }
        // 鉴权失败说明用户可订购， 构建订购地址
    ...
    public function payCallback($payResultInfo = null)
    {
        if ($payResultInfo == null) {
            LogUtils::info("Pay210092::payCallback ---> _GET: " . json_encode($_GET));
    ...
    private function _makeTransactionID($spId)
    {
        ...
        LogUtils::info("Pay210092::makeTransactionID() ---> transactionId:" . $transactionId);
    ...
    private function _buildPayUrl($userInfo, $payInfo, $productInfo)
    {
        ...
        LogUtils::info("Pay210092::_buildPayUrl() ---> infoParam: " . json_encode($infoParam));
        ...
        LogUtils::info("Pay210092::_buildPayUrl() ---> payUrl: " . $payUrl);
        return $payUrl;
    }
    ...
    private function _buildPayCallbackUrl($param = null)
    {
        ...
        LogUtils::info("Pay210092::_buildPayCallbackUrl()  payBackUrl" . $url);
        return $url;
    }
    ...
    private function _uploadPayResult($payResultInfo = null)
    {
        ...
        LogUtils::info("Pay210092::_uploadPayResult() ---> payResultInfo: " . json_encode($payResultInfo));

        if (!isset($payResultInfo['transactionID'])) {
            LogUtils::error("Pay210092::_uploadPayResult() ---> cannot find transactionID!!!!!!!");
            return 0;
        }
    ...
    private function queryUserOrderDetail($userAccount, $userToken)
    {
        ...
        LogUtils::info("Pay210092::authentication ---> authResult:" . $respond);
    ...
    public function webPagePay($userInfo) {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay210092::webPagePay ---> pay orderItem is empty");
            return $payUrl;
        }
        ...
        LogUtils::info("Pay210092::webPagePay pay ---> tradeInfo: " . json_encode($tradeInfo));
        if ($tradeInfo->result == 0) {
            // 订购的产品信息
            $productInfo = new \stdClass();
            $productInfo->productNo = productID_18;       // 产品编号
