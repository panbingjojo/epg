...
class Pay11000051 extends PayAbstract
{
    /**
    ...
    {
        LogUtils::info("Pay11000051 --> user authentication productInfo:" . json_encode($productInfo));
        ...
        LogUtils::info("Pay11000051 --> user authentication param:" . json_encode($info));

        // post到局方鉴权，同步返回数据
        $result = HttpManager::httpRequest("POST", $authorizationUrl, json_encode($info));

        LogUtils::info("Pay11000051 --> user authentication result:  " . $result);
    ...
    public function authenticationTJ()
    {
        ...
        LogUtils::info("Pay11000051 --> user authenticationTJ param:" . json_encode($info));

        // post到局方鉴权，同步返回数据
        $result = HttpManager::httpRequest("POST", $authorizationUrl, json_encode($info));

        LogUtils::info("Pay11000051 --> user authenticationTJ result:  " . $result);
    ...
    public function payCallback($payResultInfo = null)
    {
        ...
        LogUtils::info(" payCallback 11000051 ---> payResult: " . json_encode($payResultInfo));
        ...
        /*
        {
            ...
            if ($areaCode == '201') {
                if ($payResultInfo->result == 'back') {
                    LogUtils::info("Pay11000051::payCallback() ---> 积分商城返回");
                    ...
                    LogUtils::info("Pay11000051::payCallback() ---> 积分商城returnUrl：" . $jumpUrl);
        ...
        */
        ...
        $isVip = 0;
        LogUtils::info(" payCallback 11000051 ---> result: " . $payResultInfo->result);
        ...
        if ($payResultInfo->lmreason != null && $payResultInfo->lmreason == 2) {
            ...
            LogUtils::info("Pay11000051::payCallback() url:". $intentUrl);
        ...
        LogUtils::info("Pay11000051::payCallback() ---> videoInfo:" . $videoInfo);
        if ($isVip == 1 && $payResultInfo->isPlaying == 1 && $videoInfo != null) {
            // 继续播放
            LogUtils::info("Pay11000051::payCallback() ---> jump player!");
            ...
            LogUtils::info("Pay11000051::payCallback() ---> jump returnPageName: " . $payResultInfo->returnPageName);
            IntentManager::back($payResultInfo->returnPageName);
        }
    }
    ...
    private function _buildPayUrl($payInfo, $productInfo)
    {
        ...
        $orderInfo = array(
            ...
            "UserID" => $accountId,
            "UserToken" => $userToken,
            "ServiceID" => empty($productInfo->serviceId)?SERVICE_ID:$productInfo->serviceId,
            "ContentID" => empty($productInfo->contentID)?CONTENT_ID:$productInfo->contentID,
            "ProductID" => empty($productInfo->productId)?PRODUCT_ID . "@" . MasterManager::getAreaCode():$productInfo->productId,
            "Action" => '1',
            "OrderMode" => '1',
            "ContinueType" => '1',
            //"NeedRewardPoints" => '0',
    ...
    public function buildUrlAndPay($userId, $userInfo, $orderInfo)
    {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay11000051::buildUrlAndPay() ---> orderItems is empty");
            return;
        }
        ...
        LogUtils::info("Pay11000051::buildUrlAndPay() ---> authentication result: " . $authInfo->result);
        if ($authInfo->result === 10020001) {
            //鉴权失败，保存鉴权返回的订购信息项。
            SessionManager::setUserSession(SessionManager::$S_ORDER_ITEM, $authInfo);
            $authProductInfo = $authInfo->productList[0];
            // 创建订单号
            LogUtils::info("Pay11000051::buildUrlAndPay() ---> authProductInfo:" . json_encode($authProductInfo));
            ...
            if ($tradeInfo->result != 0 || $tradeInfo->order_id == null || $tradeInfo->order_id == "") {
                LogUtils::info("Pay11000051::buildUrlAndPay() ---> 拉取订单失败:" . $tradeInfo->result);
                return;
            }
            ...
            if (!$isVip) {
                if (UserManager::regVip($userId) == 1) {
                    LogUtils::info("Pay11000051::buildUrlAndPay() ---> regVip  success!");
                    $isVip = 1;
                }
                LogUtils::info("Pay11000051::buildUrlAndPay() --->regVip failed!");
            }
            SessionManager::setUserSession(SessionManager::$S_IS_VIP_USER, $isVip);

            IntentManager::back();
        } else {
            LogUtils::info("Pay11000051::buildUrlAndPay() ---> user exception");
        }
    }
    ...
    public function buildDirectPayUrl() {
        ...
        LogUtils::info("Pay11000051::buildDirectPayUrl() --->  userInfo: " . json_encode($userInfo));
        //拉取订购项
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay11000051::buildDirectPayUrl() ---> orderItems is empty");
            return $payUrl;
        }
        ...
        $authInfo = $this->authentication($productInfo);
        LogUtils::info("Pay11000051::buildDirectPayUrl() ---> authentication result: " . $authInfo);
        if ($authInfo->result === 10020001) {
            //鉴权失败，保存鉴权返回的订购信息项。
            SessionManager::setUserSession(SessionManager::$S_ORDER_ITEM, $authInfo);
            ...
            if ($tradeInfo->result != 0 || $tradeInfo->order_id == null || $tradeInfo->order_id == "") {
                LogUtils::info("Pay11000051::buildDirectPayUrl() ---> 拉取订单失败:" . $tradeInfo->result);
                return $payUrl;
            }
            $orderInfo->tradeNo = $tradeInfo->order_id;
            ...
            // 在局方已经是VIP
            LogUtils::info("Pay11000051::buildDirectPayUrl() ---> auth result = 0");
        } else {
            LogUtils::info("Pay11000051::buildDirectPayUrl() ---> user exception");
        }
        if(!empty($payUrl)){
    ...
    public function buildPointExchangeUrl($orderInfo = null) {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay11000051::buildPointExchangeUrl() ---> orderItems is empty");
        }
        ...
        if ($tradeInfo->result != 0 || $tradeInfo->order_id == null || $tradeInfo->order_id == "") {
            LogUtils::info("Pay11000051::buildPointExchangeUrl() ---> 拉取订单失败:" . $tradeInfo->result);
            return null;
        }
        $orderInfo->tradeNo = $tradeInfo->order_id;
        ...
        LogUtils::info("Pay11000051::buildPointExchangeUrl() ---> _buildPointExchangeUrl" );
    ...
    public function _buildPointExchangeUrl($param=null) {
        ...
        LogUtils::info("Pay11000051::_buildPointExchangeUrl areaCode:" . $areaCode);
