...
class Pay650092 extends PayAbstract
{
    /**
     * 到局方鉴权，只有包月产品才能鉴权。
    ...
    public function authentication($param = null)
    {
        ...
        LogUtils::info("Pay650092::authUrl --->:" . $authUrl);
        $res = HttpManager::httpRequest("GET", $authUrl, "");
        LogUtils::info("Pay650092::authentication ---> authResult:" . $res);
        ...
        if (isset($resArr["respCode"])) {
            if ($resArr["respCode"] === 0 || $resArr["respCode"] === "0") {
                if ($resArr["ordered"] === 1 || $resArr["ordered"] === "1") {
                    ...
                    LogUtils::info("Pay650092::authentication ---> is free experience!!!");
                }
            }
        }
        LogUtils::info("Pay650092::authentication ---> isVip:" . $isVip);
        return $isVip;
    }
    ...
    public function payCallback($payResultInfo = null)
    {
        ...
        if($this->authentication()){
            if($orderInfo['result'] != 0){
                LogUtils::error("Pay::authentication 订购结果与鉴权结果不符!"."订购结果：".$orderInfo['result']."鉴权结果：1");
                $orderInfo['result'] = 0;
            }
        }else{
            if($orderInfo['result'] == 0){
        ...
        LogUtils::info("callback650092UI --->orderInfo:" . json_encode($orderInfo));
        ...
        if ($lmreason == 1 || $lmreason == 2) {
            ...
            LogUtils::info("Pay650092::payCallback() ---> jump " . $returnPageName);
