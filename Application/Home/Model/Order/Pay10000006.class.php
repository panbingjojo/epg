...
class Pay10000006 extends PayAbstract
{
    /**
    ...
    {
        LogUtils::info("Pay10000006 --> user authentication productInfo:" . json_encode($productInfo));
        ...
        LogUtils::info("Pay10000006 --> user authentication param:" . json_encode($info));
        ...
        LogUtils::info("Pay10000006 --> user authentication result:  " . $result);
    ...
    public function buildPayInfo($payInfo = null)
    {
        ...
        LogUtils::info("Pay10000006::payInfo：" . json_encode($payInfo));
    ...
    public function buildUrlAndPay($userId, $userInfo, $orderInfo)
    {
        ...
        if ($authInfo->result === 10020001) {
            ...
            LogUtils::info("Pay10000006::buildUrlAndPay authProductInfo: " . json_encode($authProductInfo));
            ...
            if ($tradeInfo->result != 0 || $tradeInfo->order_id == null || $tradeInfo->order_id == "") {
                LogUtils::info("Pay10000006::buildUrlAndPay() ---> 拉取订单失败:" . $tradeInfo->result);
                return null;
            }
            ...
            if (!$isVip) {
                if (UserManager::regVip($userId) == 1) {
                    LogUtils::info("Pay10000006::buildUrlAndPay() ---> regVip  success!");
                    $isVip = 1;
                }
                LogUtils::info("Pay10000006::buildUrlAndPay() --->regVip failed!");
            }
            MasterManager::setUserIsVip($isVip);

            IntentManager::back();
        } else {
            LogUtils::info("Pay10000006::buildUrlAndPay() ---> user exception[result = $authInfo->result]");
        }
    ...
    private function _buildPayUrl($payInfo, $productInfo)
    {
        ...
        LogUtils::info("Pay10000006::buildUrlAndPay productInfo: " . json_encode($productInfo));
        // 组装订购参数
        $orderInfo = array(
            "SPID" => $productInfo->spId,
            "UserID" => $accountId,
    ...
    {
        LogUtils::info("payCallback10000006 ---> REQUEST_payResult:" . json_encode($_REQUEST));
    ...
    public function buildDirectPayUrl()
    {
        ...
        LogUtils::info("Pay10000006::buildDirectPayUrl() ");
        //拉取订购项
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay10000006::buildDirectPayUrl() ---> orderItems is empty");
            return $payUrl;
        }
        ...
        $authInfo = $this->authentication($productInfo);
        LogUtils::info("Pay10000006::buildDirectPayUrl() ---> authentication result: " . $authInfo);
        if ($authInfo->result === 10020001) {
            //鉴权失败，保存鉴权返回的订购信息项。
            MasterManager::setOrderItems($authInfo);
            ...
            if ($tradeInfo->result != 0 || $tradeInfo->order_id == null || $tradeInfo->order_id == "") {
                LogUtils::info("Pay10000006::buildDirectPayUrl() ---> 拉取订单失败:" . $tradeInfo->result);
                return $payUrl;
            }
            $orderInfo->tradeNo = $tradeInfo->order_id;
            ...
            // 在局方已经是VIP
            LogUtils::info("Pay10000006::buildDirectPayUrl() ---> auth result = 0");
        } else {
            LogUtils::info("Pay10000006::buildDirectPayUrl() ---> user exception");
        }
