...
class Pay500094 extends PayAbstract
{
    /**
     * 用户到局方鉴权
     * @param $param
    ...
    public function buildPayUrl($payInfo = null)
    {
        ...
        // 得到EPG缓存信息
        $epgInfoMap = MasterManager::getEPGInfoMap();
        LogUtils::info("Pay500094::buildPayUrl() ---> epgInfoMap: " . json_encode($epgInfoMap));
    ...
    public function directToPay($orderInfo = null)
    {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay500094::directToPay() ---> orderItems is empty");
            IntentManager::back();
            exit();
        }
        // 直接订购，使用第一个订购项（包月订购项）。
        ...
        if ($tradeInfo->result != 0) {
            // 生成订购单号失败
            LogUtils::info("Pay500094::directToPay() ---> 拉取订单失败:" . $tradeInfo->result);
            IntentManager::back();
            exit();
        }
        // 鉴权失败说明用户可订购， 构建订购地址
    ...
    public function payCallback($payResultInfo = null)
    {
        if ($payResultInfo == null) {
            LogUtils::info("Pay500094::payCallback ---> _GET: " . json_encode($_GET));
            ...
            LogUtils::info("Pay500094::payCallback ---> payResultInfo: " . json_encode($payResultInfo));
        }
        ...
        if (($payResultInfo->lmreason != null && ($payResultInfo->lmreason == 2 || $payResultInfo->lmreason == 1))
            ...
            LogUtils::info("Pay500094::payCallback() url:". $intentUrl);
    ...
    private function _makeTransactionID($spId)
    {
        ...
        LogUtils::info("Pay500094::makeTransactionID() ---> transactionId:" . $transactionId);
    ...
    private function _buildPayUrl($userInfo, $payInfo, $productInfo)
    {
        ...
        LogUtils::info("Pay500094::buildPayUrl() ---> payInfo: " . json_encode($payInfo));
        LogUtils::info("Pay500094::buildPayUrl() ---> param: " . json_encode($param));
        ...
        LogUtils::info("Pay500094::_buildPayUrl() ---> payUrl: " . $payUrl);
        return $payUrl;
    }
    ...
    private function _buildPayCallbackUrl($param = null)
    {
        ...
        LogUtils::info("Pay500094::_buildPayCallbackUrl()  payBackUrl" . $url);
        return $url;
    }
    ...
    private function _uploadPayResult($payResultInfo = null)
    {
        ...
        LogUtils::info("Pay500094::_uploadPayResult() ---> payResultInfo: " . json_encode($payResultInfo));

        if (!isset($payResultInfo['transactionID'])) {
            LogUtils::error("Pay500094::_uploadPayResult() ---> cannot find transactionID!!!!!!!");
            return 0;
        }
    ...
    private function queryUserOrderDetail($userAccount, $userToken)
    {
        ...
        LogUtils::info("Pay500094::authentication ---> authResult:" . $respond);
    ...
    public function webPagePay($userInfo) {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay500094::webPagePay ---> pay orderItem is empty");
            return $payUrl;
        }
        ...
        LogUtils::info("Pay500094::webPagePay pay ---> tradeInfo: " . json_encode($tradeInfo));
        if ($tradeInfo->result == 0) {
            // 订购的产品信息
            $productInfo = new \stdClass();
            $productInfo->productNo = productID_18;       // 产品编号
    ...
    public function buildDirectPayUrl() {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay500094::buildDirectPayUrl() ---> orderItems is empty");
            return $payUrl;
        }
        // 直接订购，使用第一个订购项（包月订购项）。
        ...
        if ($tradeInfo->result != 0) {
            // 生成订购单号失败
            LogUtils::info("Pay500094::buildDirectPayUrl() ---> 拉取订单失败:" . $tradeInfo->result);
            return $payUrl;
        }
        // 鉴权失败说明用户可订购， 构建订购地址
        $orderInfo->tradeId = $tradeInfo->order_id;
        ...
        $payUrl = $this->_buildPayUrl($userInfo, $orderInfo, $productInfo);
        // 跟上我方的订单号：
        LogUtils::info("Pay500094::buildDirectPayUrl() ---> payUrl:" . $payUrl);
        if (!empty($payUrl)) {
