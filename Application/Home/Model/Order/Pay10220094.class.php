...
class Pay10220094 extends PayAbstract
{
    ...
    public function payCallback($payResultInfo = null)
    {
        ...
        self::c_func_log(__FILE__, __FUNCTION__, ">>> [吉林广电魔方局方订购页]返回的result取值: " . $payResultInfo->result);
        ...
        if ($isOrderSuccess) {
            ...
            self::c_func_log(__FILE__, __FUNCTION__, "判断用户是否是VIP，更新到session中 isVip=$isVip");
    ...
    public function buildDirectPayUrlEx()
    {
        ...
        LogUtils::info("Pay10220094::buildDirectPayUrl ---> start");
        $payUrl = "";
        ...
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay10220094::buildDirectPayUrl ---> pay orderItem is empty");
            return $payUrl;
        }
        // 获取订购项
        $orderNoJson = OrderManager::createPayTradeNo( $orderItems[0]->vip_id, $orderReason, $remark, null, $orderType);  // 向CWS获取订单号
        LogUtils::info("Pay10220094::buildDirectPayUrl ---> user [ . $userId . ]request transactionID --> result:" . json_encode($orderNoJson));
    ...
    private function _authProduct($account){
        $contentId = "unknown";
        $products = "2010000335-2010000333-2010000332"; // 2010000335:包年两个产品包 2010000332 续保月，2010000333 单月
        $userId = $account;
        ...
        LogUtils::info("Pay10220094::_authProduct() ---> httpRequest url:" . $authURL);
        $result = HttpManager::httpRequest("GET", $authURL, "");
        LogUtils::info("Pay10220094::_authProduct() ---> httpRequest result:" . json_encode($result));
    ...
    private function _ordersPrecreate($account){
        ...
        $param = array(
            "UserId" => $account,
            "productId" => "2010000211",//"2010000332",//连续包月-2010000212-2010000213
            "contentId" => "unknown",
            "contentName" => "()",
            "ColumnId" => "",
        );
        $url = "http://139.215.93.156:9080/valuedas/api/1.0/Orders/Precreate";
        LogUtils::info("Pay10220094::_ordersPrecreate() ---> httpRequest url:" . $url);
        $result = HttpManager::httpRequestByHeader("POST", $url, $header, json_encode($param));
        LogUtils::info("Pay10220094::_ordersPrecreate() ---> httpRequest result:" . json_encode($result));
    ...
    private function _buyOrder($ordersInfo){
        ...
        $fir = "unknown";
        $pid = "unknown";
        $contentName = urlencode("健康魔方（连续包月）");
        ...
        LogUtils::info("Pay10220094::_buyOrder() ---> httpRequest url:" . $url);
        $result = HttpManager::httpRequestByHeader("POST", $url, $header, "");
        LogUtils::info("Pay10220094::_buyOrder() ---> httpRequest result:" . json_encode($result));
    ...
    private function _webpayPay($ordersInfo, $buyOrderInfo, $succCallbackUrl, $failCallbackUrl){
        ...
        $succCallbackUrl = str_replace("&", "&amp;", $succCallbackUrl);
        $failCallbackUrl = str_replace("&", "&amp;", $failCallbackUrl);
        LogUtils::info("Pay10220094::webpayPay() ---> httpRequest url succCallbackUrl:" . $succCallbackUrl);
        LogUtils::info("Pay10220094::webpayPay() ---> httpRequest url failCallbackUrl:" . $failCallbackUrl);
        ...
        LogUtils::info("Pay10220094::webpayPay() ---> httpRequest url:" . $url);
        LogUtils::info("Pay10220094::webpayPay() ---> httpRequest url xml:" . $xml);
        $result = HttpManager::httpRequestByHeader("POST", $url, $header, $xml);
        LogUtils::info("Pay10220094::webpayPay() ---> httpRequest result:" . json_encode($result));
    ...
    private function _buildWebpayUrl($webpayInfo){
        ...
        LogUtils::info("Pay10220094::_buildWebpayUrl() ---> httpRequest xml:" . $xml);
        LogUtils::info("Pay10220094::_buildWebpayUrl() ---> httpRequest json(xml):" . json_encode($xml));
        $webPayRoot = (string) $xml->webUrlIp;
        $flag = (string) $xml->flag;
        if (!empty($webPayRoot) && !empty($flag)) {
            $url = $webPayRoot . "/pay_options?flag=" . $flag;
        }
        LogUtils::info("Pay10220094::_buildWebpayUrl() ---> httpRequest url:" . $url);
        return $url;
    }
    ...
    private function _queryMyorders($acoount){
        ...
        $url = $url . "/" . $acoount;
        LogUtils::info("Pay10220094::_queryMyorders() ---> httpRequest url:" . $url);
        $result = HttpManager::httpRequest("GET", $url, "");
        LogUtils::info("Pay10220094::_queryMyorders() ---> httpRequest result:" . json_encode($result));
        if (!empty($result)) {
            $result = json_decode($result);
            $data = $result->list;
            if ($result->result == 1 && $result->count > 0 && count($data) > 0) {
                for ($index = 0; $index < count($data) ;  $index++) {
                    if ($data[$index]->productId == "2010000211") {//连续包月
                        // 找到订购的产品
                        $time=date("Y-m-d H:i:s");
                        LogUtils::info("Pay10220094::_queryMyorders() ---> endTime: " . $data[$index]->endTime);
                        if(strtotime($data[$index]->endTime)-strtotime($time) > 0){                   //对两个时间差进行差运算
                            LogUtils::info("Pay10220094::_queryMyorders() ---> data: " . json_encode($data[$index]));
                            return $data[$index];
                        }
                    }
                }
