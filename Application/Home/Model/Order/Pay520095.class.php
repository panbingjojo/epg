 ...
 */
class Pay520095 extends PayAbstract
{
    /**
     * 订购鉴权
     * @param null $payInfo
    ...
    public function authentication($payInfo = null)
    {
        ...
        LogUtils::info("Pay520095 ---> authentication isSuccess : " . $isSuccess);
        return $isSuccess;
    }
...
//        $data['Token'] = $userToken;
        LogUtils::info("Pay520095 ---> _authentication result data: " . json_encode($data));
        $resultData = HttpManager::httpRequest("POST", AUTH_URL, json_encode($data));
        LogUtils::info("Pay520095 ---> _authentication result : " . $resultData);
        $resultObj = json_decode($resultData);
        if ($resultObj->Result != 1) {
            $isSuccess = 1;
        }
        LogUtils::info("Pay520095 ---> _authentication isSuccess : " . $isSuccess);
        return $isSuccess;
    }
...
//        $authUrl = NEW_AUTH_URL . CONTENT_DP_ID .MasterManager::getAccountId() .CONTENT_NAME. '&providerId='.VENDORC_CODE . '&orderInfo=' . urlencode($orderInfo);

        LogUtils::info("Pay520095::authUrl --->:" . $authUrl);
        $res = HttpManager::httpRequest("GET", $authUrl, "");
        LogUtils::info("Pay520095::authentication_new ---> authResult:" . $res);
        ...
        if (isset($resArr["respCode"])) {
            if ($resArr["respCode"] === 0 || $resArr["respCode"] === "0") {
                if ($resArr["ordered"] === 1 || $resArr["ordered"] === "1") {
                    ...
                    LogUtils::info("Pay520095::authentication_new ---> is free experience!!!");
                }
            }
        }
        LogUtils::info("Pay520095::authentication_new ---> isVip:" . $isVip);
...
//        if($epgInfoMap["edition"] == "2" || $epgInfoMap["platform"] == "20" || MasterManager::getAccountId()=="dcgs03@itv1"){
        $ret['result'] = 0;
...
//        }
        ...
        if ($result) {
            ...
            if ($orderInfo->result != 0) {
                ...
                LogUtils::info("pay520095::buildPayUrl() ---> 获取订单失败：" . $ret['result']);
                return json_encode($ret);
            }
...
//        $payInfo->videoInfo = "undefined";
    ...
    public function payCallback($payResultInfo = null)
    {
        ...
        LogUtils::info(" payCallback 520095 ---> payResult: " . json_encode($payResultInfo));
        ...
        if ($payResultInfo->lmreason != null && (($payResultInfo->lmreason == 2) || ($payResultInfo->lmreason == 1))){
            ...
            LogUtils::info("Pay520095::payCallback() url:". $intentUrl);
        ...
        if ( $isVip == 1 && $payResultInfo->isPlaying == 1 && $videoInfo != null && $videoInfo != "undefined" ) {
            ...
            if ($payResultInfo->Result != "1") {
                LogUtils::info(" payCallback 520095 ---> 1 payResultInfo->Result : " . $payResultInfo->Result);
                if (empty($payResultInfo->returnPageName)) {
                    IntentManager::back("");
                } else {
                    // 失败直接返回，不走结果显示页面
                    LogUtils::info(" payCallback 520095 ---> returnPageName: " . $payResultInfo->returnPageName);
                    IntentManager::jump($intent, null, IntentManager::$INTENT_FLAG_NOT_STACK);
                }
            } else {
                LogUtils::info(" payCallback 520095 ---> 2 payResultInfo->Result: " . $payResultInfo->Result);
...
//            "SPID" => SPID,                                                     // 商户号
        ...
        LogUtils::info("Pay520095 ---> directToPay result : " . $resultData);
        $resultObj = json_decode($resultData);
        if ($payAction == 2) {
            //退订时，直接得到结果
            return $resultObj;
    ...
    public function buildDirectPayUrl() {
        ...
        if ($result) {
            //鉴权成功，表示用户是订购过的
            LogUtils::error("Pay520095::buildDirectPayUrl  ---> authentication user had order!!!!");
            return $payUrl;
        }
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay520095::buildDirectPayUrl ---> getOrderItem pay orderItem is empty");
            return $payUrl;
        }
        $orderInfo->vipId = $orderItems[0]->vip_id;
        for ($index=0; $index < count($orderItems); $index++) {
        ...
        if ($tradeInfo->result != 0) {
            // 创建失败
            LogUtils::info("Pay520095::buildDirectPayUrl ---> createPayTradeNo 获取订单失败：" . $tradeInfo->result);
            return $payUrl;
        }
        $orderInfo->tradeNo = $tradeInfo->order_id;
        ...
        if (empty($payUrl)) {
            // 创建失败
            LogUtils::info("Pay520095::buildDirectPayUrl  ---> _buildPayUrl() 获取订单url失败：");
            return $payUrl;
        }
    ...
    public function buildPayUrl_New($payInfo = null)
    {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay520095::buildPayUrl_New ---> getOrderItem pay orderItem is empty");
        }
        ...
        if ($orderNoJson->result == 0) {
            ...
            //$retArr["returnUrl"] = PAY_RETURN_URL;
    ...
    private function getOurOrderCallback($returnPageName, $reason, $isPlaying = 0, $orderId, $isAsync = false)
    {
        ...
        $urlParam = "?userId=" . $userId
            . "&lmcid=" . CARRIER_ID_GUIZHOUGD_XMT
            . "&lmreason=" . $reason
