...
class Pay640092 extends PayAbstract
{
    ...
    {
        LogUtils::info("Pay640092 --> direct go pay!!!");
        $userId = MasterManager::getUserId();
        if ($orderInfo == null) {
        ...
        if (count($orderItems) <= 0) {
            //TODO 错误处理
            LogUtils::error("Pay640092::directToPay() ---> orderItems is empty");
            IntentManager::back();
            exit();
        }
        // 直接订购，使用第一个订购项（包月订购项）。
        ...
        if ($tradeInfo->result != 0) {
            // 生成订购单号失败
            LogUtils::info("Pay640092::directToPay() ---> 拉取订单失败:" . $tradeInfo->result);
            IntentManager::back();
            exit();
        }
        // 鉴权失败说明用户可订购， 构建订购地址
    ...
    public function payCallback($payResultInfo = null)
    {
        ...
        $str = rtrim($str, '&');
        LogUtils::info("callback640092UI ---> _GET: " . $str);
        ...
        if ($lmReason == 2 || $lmReason == 1) {
            LogUtils::info("Pay640092::payCallback640092() ---> lmreason:" . $lmReason);
        ...
        if (($isVip == 1) && ($isPlaying == 1)) {
            LogUtils::info("Pay640092::payCallback() ---> jump player!");
            ...
            LogUtils::info("Pay640092::payCallback() ---> jump returnPageName: " . $returnPageName);
    ...
    private function _buildPayUrl($userInfo, $payInfo, $productInfo)
    {
        ...
        LogUtils::info("Pay640092::_buildPayUrl() ---> infoParam: " . json_encode($infoParam));
        ...
        LogUtils::info("Pay640092::_buildPayUrl() ---> payUrl: " . $payUrl);
        return $payUrl;
    }
    ...
    private function _buildPayCallbackUrl($param = null)
    {
        ...
        LogUtils::info("Pay640092::_buildPayCallbackUrl()  payBackUrl" . $url);
        return $url;
    }
    ...
    public function webPagePay($userInfo) {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay640092::webPagePay ---> pay orderItem is empty");
            return $payUrl;
        }
        ...
        LogUtils::info("Pay640092::webPagePay pay ---> tradeInfo: " . json_encode($tradeInfo));
        if ($tradeInfo->result == 0) {
            // 订购的产品信息
            $productInfo = new \stdClass();
            $productInfo->productNo = productID_15;       // 产品编号
    ...
    public function buildDirectPayUrl() {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("Pay640092::buildDirectPayUrl ---> pay orderItem is empty");
            return $payUrl;
        }
        ...
        // 创建订单
        $tradeInfo = OrderManager::createPayTradeNo($orderInfo->vipId, $orderInfo->orderReason, $orderInfo->remark, "", $orderInfo->orderType); // 向CWS获取订单号
        LogUtils::info("Pay640092::buildDirectPayUrl pay ---> tradeInfo: " . json_encode($tradeInfo));
        if ($tradeInfo->result == 0) {
            // 订购的产品信息
            $productInfo = new \stdClass();
            $productInfo->productNo = productID_15;       // 产品编号
