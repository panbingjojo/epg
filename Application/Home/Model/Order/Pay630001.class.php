...
class Pay630001 extends PayAbstract
{
    ...
    public function authentication($userInfo = null)
    {
        ...
        $productInfo->copyrighted = COPYRIGHT_ID;
        ...
        if (!empty($result)) {
            ...
            if ((string)$xmlObj->body->authorize['result'] == '0') {
                ...
                MasterManager::setVIPUser(true);
                MasterManager::setOrderResult(true);
    ...
    public function buildPayInfo($payInfo = null)
    {
        ...
        if ($orderInfo->result != 0) {
            ...
            $ret['message'] = "创建订单失败";
            LogUtils::info("pay630001::buildPayUrl() ---> 获取订单失败：" . $ret['result']);
...
//            LogUtils::error("Pay630001::buildAutoPayUrl ---> orderItem is empty");
//            return $payInfo;
//        }
//
//        // 去第一个，默认包月对象
...
//        LogUtils::info("Pay630001:: payInfoParam is $payInfoParam");
...
//    }
    ...
    public function payCallback($payResultInfo = null)
    {
        if ($payResultInfo == null) {
            ...
            LogUtils::info(" payCallback 630001 ---> payCallback===>param session: " . MasterManager::getPayCallbackParams());
        ...
        LogUtils::info(" payCallback 630001 ---> payResult: " . json_encode($payResultInfo));
        ...
        LogUtils::info(" payCallback 630001 ---> showOrderResult url: " . $url);

        if ($isVip == 1 && $payResultInfo->isPlaying == 1 && $videoInfo != null) {
            // 订购成功，且有播放视频信息，那么将播放器压入Intent栈
            LogUtils::info(" payCallback 630001 ---> player: ");
            ...
            LogUtils::info(" payCallback 630001 ---> not player: ");
    ...
    public function interfacePay($userInfo)
    {
        ...
        $orderItems = OrderManager::getOrderItem($userId);
        if (count($orderItems) <= 0) {
            LogUtils::error("pay630001Action::buildAutoPayUrl ---> orderItem is empty");
        ...
        if ($orderInfo->result != 0) {
            ...
            $ret['result'] = $orderInfo->result;
            $ret['message'] = "创建订单失败";
            LogUtils::info("pay630001Action::buildPayUrl() ---> 获取订单失败：" . $ret['result']);
            return json_encode($ret);
        }
        //获取订单成功
         ...
         */
        LogUtils::error("pay630001Action:" . $payResult);
        return $payResult;
    }
    ...
    public function scspProxyAuthorize($productInfo) {
        ...
        LogUtils::info("Pay630001::scspProxyAuthorize ---> request: " . SCSP_PROXY_URL . $requestXml);
        $result = HttpManager::httpRequestByHeader("POST", SCSP_PROXY_URL, $header, $requestXml);
        LogUtils::info("Pay630001::scspProxyAuthorize ---> result:" . $result);
        return $result;
    }
    ...
    public function scspProxyADVPay($payInfo) {
        ...
        LogUtils::info("Pay630001::scspProxyADVPay ---> request: " . SCSP_PROXY_URL . $requestXml);
        $result = HttpManager::httpRequestByHeader("POST", SCSP_PROXY_URL, $header, $requestXml);
        LogUtils::info("Pay630001::scspProxyADVPay ---> result:" . $result);
        if ($result) {
            $xml = simplexml_load_string($result);
