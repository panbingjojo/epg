...
if(RenderParam.carrierId != '10000051') {
    ...
    var defaultFocusId = "btn-video"; //默认焦点
}
...
var Page = {
    ...
    getCurrentPage: function () {
        ...
        currentPage.setParam('navIndex', Collection.keepNavFocusId);
...
var Collection = {
        ...
        isLeaveNav: false,
        lastFocusId: "",
        keepNavFocusId: LMEPG.Func.getLocationString('navIndex') || 'btn-video',    //默认第一个导航健康视频,
        isFirst: true,
        ...
        init: function () {
            // RenderParam.isVip=1;
            if (RenderParam.isVip == 1 || RenderParam.isVip == '1' || RenderParam.carrierId == '10000051') {
                Collection.initData();                   //初始化收藏数据，之后才初始化收藏内容

                LMEPG.BM.init(Collection.lastFocusId, buttons, "", true);
                LMEPG.ButtonManager.setSelected(Collection.keepNavFocusId, true);
            } else {
                LMEPG.BM.init('', [], '', true);
                LMEPG.UI.commonDialog.showV2("本功能仅限VIP使用！", ['成为VIP', '取消'], function (index) {
                    if (index == 0) {
        ...
        initData: function () {
            ...
            switch (Collection.keepNavFocusId) {
                case 'btn-video':
                    item_type = 1;
                    break
                case 'btn-video-gather':
                    item_type = 2;
                    break
                case 'btn-self-test':
                    item_type = 8;
                ...
                default:
                    item_type = 2;
                    Collection.keepNavFocusId='btn-video-gather';
                    //LMEPG.ButtonManager.setSelected(Collection.keepNavFocusId, true);
                    RenderParam.focusIndex=Collection.keepNavFocusId;
                    break;
            };
            Collection.getCollectData(item_type);
            Collection.initBtn(item_type);
            Collection.lastFocusId = LMEPG.Func.isExist(RenderParam.focusIndex) ? RenderParam.focusIndex : defaultFocusId; //判断初始化焦点位置
        },
        /**
         *  刷新数据按钮
         */
        reflashData: function () {
            ...
            switch (Collection.keepNavFocusId) {
                case 'btn-video':
                    item_type = 1;
                    RenderParam.focusIndex = 'btn-video';
                    break
                case 'btn-video-gather':
                    item_type = 2;
                    RenderParam.focusIndex = 'btn-video-gather';
                    break
                case 'btn-self-test':
                    item_type = 8;
                    RenderParam.focusIndex = 'btn-self-test';
                    break
                case 'btn-big-expert':
                    item_type = 4;
                    RenderParam.focusIndex = 'btn-big-expert';
                    break
            };
            Collection.getCollectData(item_type);
            Collection.initBtn(item_type);
                ...
                try {
                    ...
                    if (result == 0) {
                        ...
                        if (LMEPG.Func.isExist(collectData.list) && collectData.list.length > 0) {
                            ...
                            if (!LMEPG.Func.isEmpty(RenderParam.focusIndex) && LMEPG.BM.getButtonById(RenderParam.focusIndex) == null
                                && !Collection.isLeaveNav) {
                                var focusIndex = RenderParam.focusIndex;
                                var posNum = parseInt(focusIndex.substr(-1));
                                if (posNum > 1) {
                                    LMEPG.BM.requestFocus('item_container_' + (posNum - 1));
                                } else {
                                    if (List.pageCurrent > 1) {
                                        List.preMenu(type, 1);
                                        LMEPG.BM.requestFocus('item_container_6');
                                    } else {
                                        LMEPG.BM.requestFocus(Collection.keepNavFocusId);
                                    }
                                }
                            };
                        }else {
                            Collection.initRecommend();
                        }
                    } else {
                        Page.showError("获取收藏数据失败[code=" + result + "]");
                    }
                } catch (e) {
        ...
        initBtn: function () {
            ...
            buttons.push({
                ...
                type: 'img',
                focusable: true,
                // nextFocusDown: 'btn-video',
                nextFocusDown: 'btn-video-gather',
                nextFocusLeft: 'btn-video-gather',
                click: Page.jumpSearchPage,
                backgroundImage: g_appRootPath + "/Public/img/hd/Collect/V20/bg_search.png",
                focusImage: g_appRootPath + "/Public/img/hd/Collect/V20/f_search.png",
                focusChange: Collection.navFocusChange,
            }, {
                id: 'btn-video',
                name: '健康视频',
                type: 'img',
                focusable: true,
                nextFocusUp: 'btn-search',
                nextFocusDown: 'item_container_1',
                nextFocusRight: 'btn-video-gather',
                backgroundImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video.png",
                focusImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video_f.png",
                selectedImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video_offline.png",
                focusChange: Collection.navFocusChange,
                beforeMoveChange: Collection.onMoveChangeKeepNavFocus,
                c_type: 1,
                groupId: 'nav'
            }, {
                id: 'btn-video-gather',
                ...
                nextFocusUp: 'btn-search',
                nextFocusDown: 'item_container_1',
                nextFocusLeft: 'btn-video',
                //nextFocusRight: RenderParam.carrierId=="10220094" ? 'btn-big-expert':'btn-self-test',
                nextFocusRight: 'btn-search',
                backgroundImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video_gather.png",
                focusImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video_gather_f.png",
                selectedImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_video_gather_offline.png",
                focusChange: Collection.navFocusChange,
                beforeMoveChange: Collection.onMoveChangeKeepNavFocus,
                c_type: 2,
                groupId: 'nav'
                ...
                nextFocusLeft: 'btn-video',
                nextFocusRight: '',
                backgroundImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_self_test.png",
                focusImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_self_test_f.png",
                selectedImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn_self_test_offline.png",
                focusChange: Collection.navFocusChange,
                beforeMoveChange: Collection.onMoveChangeKeepNavFocus,
                c_type: 8,
                groupId: 'nav'
                ...
                nextFocusLeft: 'btn-video-gather',
                nextFocusRight: '',
                backgroundImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn-big-expert.png",
                focusImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn-big-expert-f.png",
                selectedImage: g_appRootPath + "/Public/img/hd/Collect/V20/btn-big-expert-offline.png",
                focusChange: Collection.navFocusChange,
                beforeMoveChange: Collection.onMoveChangeKeepNavFocus,
                c_type: 4,
                groupId: 'nav'
        ...
        onMoveChangeKeepNavFocus: function (dir, btn) {
            // 如果当前tab下无数据，则禁止焦点向下
            if (List.videoList.length == 0 && dir == 'down') {
                return false;
            }
            var _this = Collection;
            if (dir == 'left' || dir == 'right') {
                _this.isLeaveNav = true;        //导航栏切换标志，用于避免重复拉取数据
                dir == 'left' ? _this.keepNavFocusId = btn.nextFocusLeft:_this.keepNavFocusId = btn.nextFocusRight;
        ...
        // 导航栏焦点切换拉取不同数据
        navFocusChange: function (btn, hasFocus) {
            Collection.lastFocusId = "";
            if (hasFocus && btn.id !=="btn-search"){
                LMEPG.ButtonManager.setSelected(btn.id, true);
                //if (btn.id == Collection.keepNavFocusId && Collection.isLeaveNav) {
                    Collection.getCollectData(btn.c_type);
                //}
        }else{}
    },
    ...
    // if (LMEPG.Func.isExist(List.videoList) && List.videoList.length > 0) {
        ...
        LMEPG.BM.requestFocus(Collection.lastFocusId);
},
...
recommendedFocus: function (btn, hasFocus) {
    if (hasFocus) {
        ...
        onClickToDoctorsDetails: function (btn) {
            var clinic = G(btn.id).getAttribute('clinic_id');
            //var currentObj = LMEPG.Intent.createIntent('collect');
            var currentObj = Page.getCurrentPage();
            var jumpAgreementObj = LMEPG.Intent.createIntent('expertDetail');
            jumpAgreementObj.setParam('clinic', clinic);
...
onClickRecommendPosition: function (btn) {
    ...
    if (LMEPG.Func.isExist(List.videoList) && index < List.videoList.length) {
        ...
        if (itemBtnStatus == 0) {
            if(videoData.item_type == 4){
                Collection.onClickToDoctorsDetails(btn);
            }else  if (videoData.item_type == 1) {
                //播放视频
            ...
            } else if (videoData.item_type == 2) {
                //跳转到专辑详情界面
                Page.jumpChannelVideoSet(videoData.subject_id);
            } else {
                //跳转到健康自测界面
                Page.jumpTestAnswer(videoData.topic_id);
            }
...
doCollectItem: function (data, status) {
    ...
    switch (data.item_type) {
        case 1:
            ...
            break;
        case 2:
            item_id = data.subject_id;
            break;
        case 8:
            item_id = data.topic_id;
            break;
        case 4:
            item_id = data.doctor_user_id;
            break;
    };
    var postData = {
        "type": type,
...
var List = {
    ...
    pageNum: 0,                  //总页数
    cutNum: 6,                  //每页显示多少个
    itemBtnBgNormal: g_appRootPath + '/Public/img/hd/Collect/V20/bg_key.png',
    itemBtnBgFocused: g_appRootPath + '/Public/img/hd/Collect/V20/f_key.png',
    createDoctorUrl: function (doctorId, avatarUrl, carrierID) {
        var head = {
        ...
        //从本地代理到39取医生头像
        return "http://139.215.92.20:10016/cws/39hospital/index.php" + "?head=" + JSON.stringify(head) + "&json=" + JSON.stringify(json);
    },
    /**
     * 创建视频列表
     */
    createList: function () {
        ...
        for (var i = 0; i < newArr.length; i++) {
            ...
            if(RenderParam.carrierId == '10220094' && newArr[i].item_type == 4){
                strTable += '<div id="item_container_' + (i + 1) +'" clinic_id=' + newArr[i].clinic_id+ '>';
                strTable += ' <img id="item_img_expert' + i + '" class="' + 'item_img_expert' + '" alt src=' + this.createDoctorUrl(newArr[i].doctor_user_id,newArr[i].doctor_avatar,RenderParam.carrierId) +'>';
            }else{
                strTable += '<div id="item_container_' + (i + 1) +'" >';
                strTable += ' <img id="item_img_' + i + '" class="' + "item_img" + '" src="' + addFsUrl(newArr[i].item_type == 8 ? newArr[i].img_url : newArr[i].image_url) + '" onerror="this.src=\'' + defImgUrl + '\'"/>';
            }

            if(RenderParam.carrierId != '10000051'){
                strTable += ' <div id="item_text_' + i + '"class="item_text" >'+ (newArr[i].item_type == 2 ? newArr[i].subject_name : newArr[i].title) + '</div>';
            }
    ...
    onFocusChange: function (btn, hasFocus) {
        var cIndex = parseInt(btn.id.substr(-1))-1;
        // 列表项焦点效果
        if (hasFocus) {
            ...
            LMEPG.UI.Marquee.start("item_text_"+cIndex, 10, 5, 50, "left", "scroll");
            G("item_text_"+cIndex).style.left='10px';
            G("item_text_"+cIndex).style.bottom='-10px';
        } else {
            LMEPG.CssManager.removeClass(btn.id, "collection_zoom_out");
            G(btn.id + "_filter").style.display = "none";
            G(btn.id + "_filter").innerHTML = "";
            LMEPG.UI.Marquee.stop();
            G("item_text_"+cIndex).style.left='0px';
            G("item_text_"+cIndex).style.bottom='0px';
        }
    },
            ...
            case 'up':
                if (itemBtnStatus == 1) {
                    ...
                    if (cIndex >= 1 && cIndex <= 3) {
                        // LMEPG.BM.requestFocus(LMEPG.ButtonManager.getSelectedButton("nav").id);
                        LMEPG.BM.requestFocus(Collection.keepNavFocusId);
                    } else {
    ...
    //上一页
    preMenu: function (cIndex) {
        if (List.pageCurrent > 1) {
            RenderParam.pageCurrent--;
            RenderParam.focusIndex = 'item_container_' + (parseInt(cIndex) + 2);
            Collection.initRecommend();
    ...
    nextMenu: function () {
        if (List.pageCurrent < List.pageNum) {
            RenderParam.pageCurrent++;
            RenderParam.focusIndex = 'item_container_1';
            Collection.initRecommend();
            LMEPG.BM.requestFocus('item_container_1');
        }
    },
