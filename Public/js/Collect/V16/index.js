var Collect = {
	...
	render: function() {
		...
		if (isNullData) {
			...
			this.currentData.forEach(function(t, i) {
				// 收藏按钮图片
				if (t.collectType == 1 || t.collectType == undefined) {
					var collectStatusImg =g_appRootPath+ '/Public/img/hd/Collect/V16/uncollect.png';
				} else{
					var collectStatusImg = g_appRootPath+'/Public/img/hd/Collect/V16/collect.png';
				}
				...
				htm += '<li id=focus-' + i + ' pos=' + (i + page) + '>' +
					'<img onerror="this.src=\'__ROOT__/Public/img/Common/default.png\'" src=' + image + '>' +
		...
		switch (key) {
			...
			case 'right':
				...
				var turnNextCondition = {
					video: _this.navIndex < 2 && (btn.id == 'focus-3' || btn.id == 'focus-7' || btn.id == 'uncollect-3' || btn.id == 'uncollect-7'),// 视频、视频集
					person: _this.navIndex > 1 && (btn.id == 'focus-4' || btn.id == 'uncollect-4')// 医生、专家列表翻页
				};
			...
			case 'left':
				...
				var turnPrevCondition = {
					video: _this.navIndex < 2 && (btn.id == 'focus-0' || btn.id == 'focus-4' || btn.id == 'uncollect-0' || btn.id == 'uncollect-4'),
					person: _this.navIndex > 1 && (btn.id == 'focus-0' || btn.id == 'uncollect-0')
				};
				if (turnPrevCondition.video || turnPrevCondition.person) {
					_this.prevPage(btn);
					var moveToId = _this.navIndex > 1 ? 'focus-4' : 'focus-7';
	...
	onFocusChangeBgImg: function(btn, hasFocus) {
		...
		if (hasFocus && Collect.navIndex > 1) {
			// btnElement.style.backgroundImage = 'url('+(RenderParam.platformType=='sd'?'/Public/img/sd/Unclassified/V13/doctor_f.png': g_appRootPath + '/Public/img/hd/Collect/V16/radius_f.png')+')';
			btnElement.style.backgroundImage = 'url(__ROOT__/Public/img/hd/Collect/V16/radius_f.png)';
		} else{
			//	默认
		}
	},
	createBtns: function() {
		...
		while (NAV_COUNT--) {
			this.buttons.push({
				...
				// focusImage: RenderParam.platformType=='sd'?'/Public/img/sd/Unclassified/V16/nav_f.png': g_appRootPath + '/Public/img/hd/Collect/V16/nav_f.png',
				// selectedImage: RenderParam.platformType=='sd'?'/Public/img/sd/Unclassified/V16/nav_s.png': g_appRootPath + '/Public/img/hd/Collect/V16/nav_s.png',
				focusImage:g_appRootPath+'/Public/img/hd/Collect/V16/nav_f.png',
				selectedImage:g_appRootPath+'/Public/img/hd/Collect/V16/nav_s.png',
		...
		while (VIDEO_COUNT--) {
			this.buttons.push({
				...
				backgroundImage:g_appRootPath+ '/Public/img/hd/Common/transparent.png',
				// focusImage: RenderParam.platformType=='sd'?'/Public/img/sd/Unclassified/V16/video_list_f.png': g_appRootPath + '/Public/img/hd/Collect/V16/video_list_f.png',
				focusImage:  g_appRootPath+'/Public/img/hd/Collect/V16/video_list_f.png',
				...
				backgroundImage: g_appRootPath+'/Public/img/hd/Collect/V16/uncollect.png',
				focusImage: g_appRootPath+'/Public/img/hd/Collect/V16/uncollect_f.png',
		...
		LMEPG.ajax.postAPI('Collect/setCollectStatusNew', postData, function(rsp) {
			try {
				...
				if (collectItem.result == 0) {
					if (postData.type == 0) {
						//收藏成功
						G(btnId).src =g_appRootPath+ '/Public/img/hd/Collect/V16/uncollect_f.png';
						Collect.dataList[index].collectStatus = 1;
						// 更新按钮的图片
						for (var i = 0; i < Collect.buttons.length; i++) {
							if (Collect.buttons[i].id == btnId) {
								Collect.buttons[i].backgroundImage = g_appRootPath+'/Public/img/hd/Collect/V16/uncollect.png';
								Collect.buttons[i].focusImage =g_appRootPath+ '/Public/img/hd/Collect/V16/uncollect_f.png';
								break;
							}
						}
					} else{
						//取消收藏成功
						G(btnId).src = g_appRootPath+'/Public/img/hd/Collect/V16/collect_f.png';
						Collect.dataList[index].collectStatus = 0;
						// 更新按钮的图片
						for (var i = 0; i < Collect.buttons.length; i++) {
							if (Collect.buttons[i].id == btnId) {
								Collect.buttons[i].backgroundImage = g_appRootPath+'/Public/img/hd/Collect/V16/collect.png';
								Collect.buttons[i].focusImage = g_appRootPath+'/Public/img/hd/Collect/V16/collect_f.png';
								break;
							}
						}
					}
