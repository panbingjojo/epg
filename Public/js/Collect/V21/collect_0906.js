...
var Collect = {
    ...
    hideCollect: function(){
        for ( var i = 0; i < this.NUM_PER_PAGE; i++) {
            H('btn_collect_' + i);
        }
    },
    ...
    updateCollectStatus: function(btn, status){
        ...
        G(btn.id).backgroundImage = g_appRootPath + '/Public/img/hd/Home/V21/Collect/'+ (!status ? 'btn_unCollect.png' : 'btn_collect.png');
        G(btn.id).focusImage = g_appRootPath + '/Public/img/hd/Home/V21/Collect/'+ (!status ? 'btn_unCollect_f.png' : 'btn_collect_f.png');
    },
