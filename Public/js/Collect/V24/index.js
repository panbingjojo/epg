var Collect = {
    ...
    init: function () {
        ...
        Collect.renderHead(function (){
            Collect.getCollectList(Collect.headData[Collect.navIndex].model_type,1,function () {
                console.log("init");
                Collect.render();
               ...
               if(RenderParam.focusId){
                   ...
                   G('play-'+p).src =g_appRootPath+ '/Public/img/hd/Collect/V24/play_choose.png'
               }
...
// 特殊地区去掉专家一身模块
        ...
        if (isNullData) {
            ...
            this.currentData.forEach(function (t, i) {
                ...
                if (t.collectStatus == 1 || t.collectStatus == undefined) {
                    var collectStatusImg = '/Public/img/hd/Collect/V24/cancel.png';
                } else {
                ...
                htm+=   '<div class="mask" id="mask-'+i+'" style="display: none">' +
                    '<img  class="play" id="play-' + i + '" src="__ROOT__/Public/img/hd/Collect/V24/play.png" pos=' + (i + page) + '>'+
    ...
    renderHead:function (success){
        ...
        LMEPG.ajax.postAPI('Video/getVideoClass', postData, function (rsp) {
            var headData = rsp instanceof Object ? rsp : JSON.parse(rsp);

            Collect.headData = headData.data
            var html = '';

            for(var i=0;i<headData.data.length;i++){
                html += '<p id="nav-'+i+'">'+headData.data[i].model_name+'</p>'
            }
    ...
    onMoveChangeFocusId: function (key, btn) {
        ...
        switch (key) {
            ...
            case 'up' :
                ...
                G('play-'+pos).src = g_appRootPath+'/Public/img/hd/Collect/V24/play_choose.png'
                G('uncollect-'+pos).src =g_appRootPath+ '/Public/img/hd/Collect/V24/cancel.png'
            ...
            case 'down':
                ...
                Collect.changeClock++
                if(Collect.changeClock < 3 ){
                    Collect.innerMenuFlag = false
                    G('play-'+pos).src =g_appRootPath+ '/Public/img/hd/Collect/V24/play.png'
                    G('uncollect-'+pos).src =g_appRootPath+ '/Public/img/hd/Collect/V24/cancel_choose.png'

                    if(Collect.changeClock === 2 && pos + 4 < 8){
                        G('mask-'+pos).style.display = 'none'
                        Collect.innerMenuFlag = true
                        Collect.changeClock = 0
                        LMEPG.BM.requestFocus(pos + 4 >= Collect.currentData.length?'focus-'+(Collect.currentData.length-1) :'focus-' + (pos + 4))
                        var position = pos + 4 >= Collect.currentData.length?Collect.currentData.length-1:pos + 4

                        G('play-'+position).src =g_appRootPath+ '/Public/img/hd/Collect/V24/play_choose.png'
                        G('mask-'+position).style.display = 'block'
                    }
                }
    ...
    onFocusChangeVideoList: function (btn, hasFocus) {
        ...
        if (hasFocus) {
            ...
            Collect.timer = setTimeout(function (){
                ...
                Collect.getCollectList(Collect.headData[itemType].model_type,1, function () {
                    _this.page = 0;
    ...
    showMenu:function (pre,btn){
        ...
        if(Collect.changeClock === 0){
            if(G('mask-'+prePos))
                G('mask-'+prePos).style.display = 'none'

            G('mask-'+pos).style.display = 'block'
            G('play-'+pos).src =g_appRootPath+ '/Public/img/hd/Collect/V24/play_choose.png'
            G('uncollect-'+pos).src = g_appRootPath+'/Public/img/hd/Collect/V24/cancel.png'
        }
    ...
    getCollectList: function (parentType,itemType,callback) {
        ...
        Collect.dataList = []
        console.log(Collect.dataList)

        for(var i = 1;i<3;i++){
            var postData = {
                'item_type': i, // 收藏对象类型（1视频 2视频专辑 3医生 4专家）
                ...
                if (collectItem.result == 0) {
                    if (postData.type == 0) {
                        ...
                        (Collect.backId*1-1<0)?
                            ...
                            :
                            LMEPG.BM.requestFocus('focus-'+(Collect.backId*1-1>=0?Collect.backId*1-1:'0'));

                        if(Collect.backId*1-1>=0){
                            var position = Collect.backId*1-1>=0?Collect.backId*1-1:'0'
                            G('mask-'+position).style.display = 'block'
                            G('play-'+position).src =g_appRootPath+ '/Public/img/hd/Collect/V24/play_choose.png'
                            G('uncollect-'+position).src =g_appRootPath+ '/Public/img/hd/Collect/V24/cancel.png'
