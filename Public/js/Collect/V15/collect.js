...
var Collection = {
     ...
     */
    init: function () {
        // LMEPG.Func.removeDefaultFocusbolder();      // 取消元素默认的边框
        if (RenderParam.carrierId == '520092' || RenderParam.carrierId == '09000051') {
            Collection.initData();
            return;
        }
        Collection.initData();
    ...
    getCollectVideo: function () {
        ...
        LMEPG.ajax.postAPI('UnifiedPlayer/getCollections', {pageNumber: 1}, function(rsp) {
            try {
                        ...
                        if (data.result == 0) {
                            for (var i = 0; i < data.data.length; i++) {
                                if (JSON.stringify(data.data[i]) != "{}") {
                                    List.videoList.push(data.data[i]);
                                }
                                // List.videoList.push(data.data[i]);
                            }
                ...
                LMEPG.UI.showToast('获取收藏记录数据解析异常');
            }
        }, function() {
            LMEPG.UI.showToast('获取收藏记录失败！');
            LMEPG.UI.dismissWaitingDialog();
            onBack();
        });
    },
    ...
    doCollectItem: function (data, status) {
        ...
        var contentCode = JSON.parse(data.ftp_url).gq_ftp_url;
        // 去收藏
...
var List = {
    ...
    createList: function () {
        ...
        console.log("createList tab_list:" + strTable);
