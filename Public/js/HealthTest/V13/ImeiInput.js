var InputImei = {
    ...
    keyPad: function (btn, hasFocus) { // 需定义一个父级容器（不可以body为父级）
        if (hasFocus) LMEPG.UI.keyboard.show(88, 100, 'input-number', false, 'next-step');
    },
    ...
    jumpPageInputDataPage: function () {
        ...
        var dstPage = LMEPG.Intent.createIntent('healthTestInputData');
        // dstPage.setParam('testType', RenderParam.testType);
        dstPage.setParam('type', RenderParam.testType);
