...
var Page = {
    ...
    getCurrentPage: function () {
        var currentPage = LMEPG.Intent.createIntent('doctor-code');
        return currentPage;
    },
...
var WaitStep = {
     ...
     */
    polling: function () {
        var postData = {'data_type': 1,"timestamp":curServerTime};
        LMEPG.ajax.postAPI('DeviceCheck/getlastTestMeasureRecord', postData, function (data) {
            ...
            if (data.result == 0 && data.data.length > 0) {
                ...
                if(data.status==1){
                    ...
                    url = "healthTestArchivingList";
                }
                if(url!="") {
                    // 跳转到数据归档页面
                    var objSrc = Page.getCurrentPage();
                    var objDst = LMEPG.Intent.createIntent(url);
