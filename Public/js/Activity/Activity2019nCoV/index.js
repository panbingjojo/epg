...
var NCOV = {
    ...
    init: function () {
        ...
        LMEPG.BM.init('question-1-option-1', buttons, '', true);
    },
    /**
     * 点击上一步按钮
     * @param btn
    ...
    questionNextStepShowAnswerResult: function (btn) {
        ...
        var currentPage = LMEPG.Intent.createIntent("activity-common-guide");
        var targetPage = LMEPG.Intent.createIntent("activity-result");
    ...
    clickQuestionOption: function (btn) {
        ...
        if (selectIcon.src.indexOf('_s') > -1) {
            ...
            selectIcon.src = ROOT +'/Public/img/hd/Activity/Activity2019nCoV/option.png';
        } else {
            NCOV.storeDataQuestion[btn.qIndex] += 1;
            selectIcon.src = ROOT + "/Public/img/hd/Activity/Activity2019nCoV/option_s.png"
        }
    },
    /**
     * 点击日期选择框
    ...
    scrollSelectOption: {
        ...
        render: function () {
            ...
            if (!this.data || !this.data.length) {
                LMEPG.UI.showToast('暂无查询信息！', 2, H(NCOV.modalId));
                return
            }
            switch (NCOV.inputId) {
                // 渲染交通工具列表
        ...
        click: function (btn) {
            ...
            if (txt === '全部') {
                ...
                G('input-keyword').style.backgroundImage = "url(__ROOT__/Public/img/hd/Activity/Activity2019nCoV/question2_input1.png)";
                trafficBtn.nextFocusDown = 'input-keyword';
                keywordBtn.nextFocusUp = 'input-traffic';
                keywordBtn.backgroundImage = ROOT+'/Public/img/hd/Activity/Activity2019nCoV/question2_input1.png';
                keywordBtn.focusImage = ROOT+'/Public/img/hd/Activity/Activity2019nCoV/question2_input1_f.png';
            }
            if (txt === '飞机' || txt === "火车") {
                H('select-abbreviation');
                H('select-board');
            ...
            if (txt === '长途客车/大巴' || txt === "出租车") {
                ...
                keywordBtn.nextFocusUp = 'input-traffic';
                keywordBtn.backgroundImage = ROOT+'/Public/img/hd/Activity/Activity2019nCoV/search_abbreviation_bg.png';
                keywordBtn.focusImage = ROOT+'/Public/img/hd/Activity/Activity2019nCoV/search_abbreviation_f.png';
            }
    ...
    moveModalDate: function (key, btn) {
        ...
        if (key === "up") {
            ...
            // 上一天
            if (lastStr === "day") {
                prev.innerHTML = zero(prevCount <= 1 ? 31 : prevCount -= 1);
                curr.innerHTML = zero(currCount <= 1 ? 31 : currCount -= 1);
                next.innerHTML = zero(nextCount <= 1 ? 31 : nextCount -= 1);
            }
        }
        // 向下滚动日期对象
        if (key === "down") {
            ...
            // 下一天
            if (lastStr === "day") {
                prev.innerHTML = zero(prevCount === 31 ? 1 : prevCount += 1);
                curr.innerHTML = zero(currCount === 31 ? 1 : currCount += 1);
                next.innerHTML = zero(nextCount === 31 ? 1 : nextCount += 1);
            }
        }
...
function onBack() {
    ...
    if (Search.isShowLetter) {
        LMEPG.BM.requestFocus(Search.keepaliveId);
