(function (w, d, e, r) {
    ...
    var LMActivity = {
        ...
        onInputFocus: function (btn, hasFocus) {
            if (hasFocus) {
                var left = btn.left || 100
                var top = btn.top || 88
                LMEPG.UI.keyboard.show(left, top, btn.id, btn.backFocusId, true);
            }
        },
        ...
        renderExchangeRecordList: function (exchangeRecordList, myExchangeRecordList) {
            ...
            if (myExchangeRecordList && myExchangeRecordList.length > 0 && myExchangeRecordList[0]['user_tel']) {
         ...
         * 广西电信地区特殊处理
         * */
        renderLotteryRecordListGX: function (lotteryRecordList, myLotteryRecordList) {
            var myLotteryRecord = null;
            var htm = '<marquee  id="all_marquee" scrollamount="5"  direction="up"><table id="all_table" class="marquee_table">';
            for (var i = 0; i < lotteryRecordList.length; i++) {
                htm += '<tr>'
            ...
            $('my_list_gx').innerHTML = myHtm;
            $('total_list_gx').innerHTML = htm;
            if (myLotteryRecordList &&
                myLotteryRecordList.length > 0
                && myLotteryRecordList[0]['user_tel']) {
                $('reset_tel_gx').innerHTML = myLotteryRecordList[myLotteryRecordList.length - 1]['user_tel'];
            } else {
                $('reset_tel_gx').innerHTML = "请输入有效的电话";
            }
        },
         ...
         * 广西电信地区特殊处理
         * */
        renderExchangeRecordListGX: function (exchangeRecordList, myExchangeRecordList) {
            var myExchangeRecord = null;
            var htm = '<marquee  id="all_marquee" scrollamount="5"  direction="up"><table id="all_table" class="marquee_table">';
            for (var i = 0; i < exchangeRecordList.length; i++) {
                htm += '<tr>'
            ...
            if (myExchangeRecordList && myExchangeRecordList.length > 0) {
                $('reset_tel_gx').innerHTML = myExchangeRecordList[0]['user_tel'];
            } else {
                $('reset_tel_gx').innerHTML = "请输入有效的电话";
            }
        },
        ...
        /**
         * 童心未泯，女神节驾到活动特殊渲染中奖页面逻辑
         * */
        specialRenderLotteryRecordList: function (lotteryRecordList, myLotteryRecordList) {
            var myLotteryRecord = null;
            // 当前登录用户中奖信息
            for (var i = 0; i < lotteryRecordList.length; i++) {
                if (lotteryRecordList[i]['user_account'] === r.userAccount) {
                    myLotteryRecord = myLotteryRecordList[i];
                }
            }

            var myHtm = '<marquee  id="my_marquee" scrollamount="0"  direction="up"><table id="my_table" class="marquee_table">';
            if (myLotteryRecord) {
                LMActivity.lotteryPrizeId = myLotteryRecord['prize_idx'];
                myHtm += '<tr>'
                    + '<td class="winner_account">'
                    + LMActivity.formatAccount(myLotteryRecord['user_tel'])
                    + '<td class="winner_time">'
                    + LMActivity.formatAccount(myLotteryRecord['insert_dt'])
                    + '<td class="winner_prize">' + myLotteryRecord['prize_name'];
            }
        ...
        checkButton: function () {
            LMActivity.check_code = !LMActivity.check_code
            if (LMActivity.check_code === true) {
                $('check').src = LMActivity.makeImageUrl('btn_check_f.png')
                e.BM.getCurrentButton().focusImage = LMActivity.makeImageUrl('btn_check_f.png');
                e.BM.getCurrentButton().backgroundImage = LMActivity.makeImageUrl(
                    'btn_check.png'
                );
            }else {
                $('check').src = LMActivity.makeImageUrl('btn_uncheck_f.png')
                e.BM.getCurrentButton().focusImage = LMActivity.makeImageUrl('btn_uncheck_f.png');
                e.BM.getCurrentButton().backgroundImage = LMActivity.makeImageUrl(
                    'btn_uncheck.png'
                );
            }
         ...
         * 兑奖成功页面-勾选用户隐私政策按钮
         * */
        exchangeCheckButton: function () {
            LMActivity.check_code = !LMActivity.check_code
            if (LMActivity.check_code === true) {
                $('exchangeCheck').src = LMActivity.makeImageUrl('btn_check_f.png')
                e.BM.getCurrentButton().focusImage = LMActivity.makeImageUrl('btn_check_f.png');
                e.BM.getCurrentButton().backgroundImage = LMActivity.makeImageUrl(
                    'btn_check.png'
                );
            }else {
                $('exchangeCheck').src = LMActivity.makeImageUrl('btn_uncheck_f.png')
                e.BM.getCurrentButton().focusImage = LMActivity.makeImageUrl('btn_uncheck_f.png');
                e.BM.getCurrentButton().backgroundImage = LMActivity.makeImageUrl(
                    'btn_uncheck.png'
                );
        ...
        renderExchangePrize: function (exchangePrizeList, exchangePrizeButtons, exchangeFocusId, moveType) {
            ...
            if (exchangePrizeList.length == 4) {
                ...
                $('exchange_point_4').innerHTML = exchangePrizeList[3].consume_list[0].consume_count;
            }else {
                $('exchange_point_1').innerHTML = exchangePrizeList[0].consume_list[0].consume_count;
                $('exchange_point_2').innerHTML = exchangePrizeList[1].consume_list[0].consume_count;
                $('exchange_point_3').innerHTML = exchangePrizeList[2].consume_list[0].consume_count;
        ...
        setExchangePhone: function (input, isReset) {
            if (r.lmcid == '450092') {
                if (LMActivity.check_code == true) {
                    ...
                    LMEPG.UI.showToast('请先同意用户隐私政策，并获取授权', 1);
                }
            }else {
                var userTel = $(input).innerText;
            ...
            switch (btn.id) {
                ...
                case 'btn_winner_list':
                    if (r.lmcid == '450092') {
                        switch (btn.listType) {
                            case 'exchange':
                                LMActivity.renderExchangeRecordListGX(r.exchangeRecordList.data.all_list,
                                    r.exchangeRecordList.data.list);
                                break;
                            case 'lottery':
                                LMActivity.renderLotteryRecordListGX(r.lotteryRecordList.list,
                    ...
                    break;
                    /*设置童心未泯，女神节驾到活动的特殊中奖名单页面*/
                case 'btn_winner_list_special':
                    if (r.myLotteryRecord.list.length === 0 || r.myLotteryRecord.list[0].user_tel === "") {
                        LMActivity.specialRenderLotteryRecordList(r.lotteryRecordList.list,
                            r.myLotteryRecord.list);
                        LMActivity.triggerModalButton = btn.id;
                        LMActivity.showModal({
                            id: 'no_winner_list',
                            focusId: 'btn_list_back_no'
                        });
                    }else {
                        LMActivity.specialRenderLotteryRecordList(r.lotteryRecordList.list,
                            r.myLotteryRecord.list);
                        ...
                        LMActivity.showModal({
                            id: 'winner_list',
                            focusId: 'btn_list_back'
                        });
                    }
                    break;
                case 'check':
                    LMActivity.checkButton();
                    break;
                case 'exchangeCheck':
                    LMActivity.exchangeCheckButton();
                    break;
                case 'user_privacy_policy':
                    LMActivity.showModal({
                        id: 'user_privacy_page',
                        focusId: 'btn_close_authorization',
                        onDismissListener: function (){
                            ...
                            LMActivity.triggerModalButton = 'btn_winner_list';
                            LMActivity.showModal({
                                id: 'winner_list_gx',
                                focusId: 'btn_list_submit_gx'
                            });
                    ...
                    break;
                case 'user_privacy_policy_gx':
                    Activity.closeSource = '1'
                    LMActivity.showModal({
                        id: 'user_privacy_page',
                        focusId: 'btn_close_authorization',
                        onDismissListener: function (){
                            LMActivity.showModal({
                                id: 'exchange_success_gx',
                                focusId: 'btn_exchange_submit_gx',
                                onDismissListener: function () {
                                    LMActivity.Router.reload();
                                }
                            })
                ...
                case 'btn_list_submit':
                    ...
                    break;
                case 'btn_list_submit_gx':
                    var listType1 = btn.listType ? btn.listType : 'exchange';
                    switch (listType1) {
                        case 'exchange':
                            LMActivity.setExchangePhone('reset_tel_gx', true);
                            break;
                        case 'lottery':
                            LMActivity.setLotteryPhone('reset_tel_gx', true);
                            break;
                    }
                    break;
                case 'btn_close_rule':
                ...
                case 'btn_exchange_submit':
                    ...
                    break;
                case 'btn_exchange_submit_gx':
                    LMActivity.setExchangePhone('exchange_tel_gx', false);
                    break;
                case 'btn_exchange_cancel':
                case 'btn_exchange_cancel_gx':
                    LMActivity.Router.reload();
                    break;
                ...
                case 'btn_lottery_submit':
                    ...
                    break;
                case 'btn_close_authorization':
                    if(Activity.closeSource === '1'){
                        LMActivity.showModal({
                            id: 'exchange_success_gx',
                            focusId: 'btn_exchange_submit_gx',
                            onDismissListener: function () {
                    ...
                    break;
                case 'btn_close_authorization_gx':
                    LMActivity.hideModal(LMActivity.shownModal)
                    break;
            }
        },
    ...
    LMActivity.Router = {
        ...
        reload: function () {
            ...
            LMEPG.UI.showWaitingDialog('', 2, function () {
                var dst = LMActivity.Router.getCurrentPage();
                dst.setParam('inner', r.inner);
                e.Intent.jump(dst);
            });
        },
    ...
    LMActivity.AjaxHandler = {
        ...
        getData: function (key, successFn, errorFn) {
            var params = {
                postData: {
                    "key": key,
                },
