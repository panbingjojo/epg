...
var List = {
    ...
    loadDataList: function (asyncCallback) {
        ...
        LMEPG.ajax.postAPI("DeviceCheck/getMemberArchivedRecordList", postData, function (data) {
            ...
            if (LMEPG.Func.isObject(data) && data.result == 0) {
                ...
                if (LMEPG.BM.getCurrentButton().id == "btn-4") {
                    List.createBloodListUI();
                } else {
                    List.createListUI();
                }
            } else {
    ...
    createListUI: function () {
        var htmlStr = "";
        htmlStr += '<table style="font-size: 25px;width: 90%">';
        htmlStr += '<tr>';
        htmlStr += '  <td style="text-align: center;">检测时间</td><td style="text-align: center;">状态</td>' +
            '<td style="text-align: center;">检测值</td><td style="text-align: center;">结果</td>';
        htmlStr += '</tr>';
    ...
    createBloodListUI: function () {
        var htmlStr = "";
        htmlStr += '<table style="font-size: 25px;width: 95%">';
        htmlStr += '<tr>';
        ...
        for (var i = 0; i < List.dataList.length; i++) {
            ...
            var measureType = data.paper_type;
            var measureDT = DT.format(data.measure_dt, "yyyy.MM.dd hh:mm:ss");
            var formatData = JSON.parse(data.measure_data);
            var highPressureValue=formatData.highPressureValue+"mmHG";
            var lowPressureValue=formatData.lowPressureValue+"mmHG";
            var heartRate=formatData.heartRate+"dpm";
            // var measureDataWithUnit = formatData + Measure.getUnitText(measureType);
            var measureLevel = Measure.getResultLevel(RenderParam.memberGender, measureType, formatData, data.repast_id);
