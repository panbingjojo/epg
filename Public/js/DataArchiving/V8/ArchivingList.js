...
var Page = {
    ...
    jumpHome: function () {
        var objCurrent = Page.getCurrentPage();
        var objDst = LMEPG.Intent.createIntent("home");
        LMEPG.Intent.jump(objDst, objCurrent);
    },
};
...
var List = {
    ...
    createListUI: function () {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            //如果类型是血压
            if (item.measure_type == "5") {
                var highPressureValue = JSON.parse(measureData).highPressureValue;
                var lowPressureValue = JSON.parse(measureData).lowPressureValue;
                var heartRate = JSON.parse(measureData).heartRate;
            }
            ...
            if (item.measure_type == "5") {
                htmlStr += '<div class="items_content" style="font-size: 26px">收缩:' + highPressureValue + "&nbsp;&nbsp;舒张:" + lowPressureValue + '&nbsp;&nbsp;心率:' + heartRate + '' +
            ...
            if (cIndex >= 0 && cIndex < pageList.length) {
                ...
                switch (Measure.getTypeAsInt(record.measure_type)) {
                    ...
                    case Measure.Type.URIC_ACID: //胆固醇和尿酸：使用timebucket_id
                        ...
                        break;
                    case Measure.Type.BLOOD_PRESSURE: //胆固醇和尿酸：使用timebucket_id
                        break;
                }
