...
var Page = {
    ...
    getCurrentPage: function () {
        var currentPage = LMEPG.Intent.createIntent("healthTestArchived");
        currentPage.setParam("type", RenderParam.type);
        currentPage.setParam("recordDataList", JSON.stringify(List.dataList)); //跟踪当前的检测记录数据
...
var List = {
    ...
    getUnarchivedRecordBy: function (measureId) {
        for (var i = 0, len = List.dataList.length; i < len; i++) {
            var item = List.dataList[i];
            if (item.measure_id == measureId) {
                return item;
            }
        }
    },

    deleteAndUpdateUI: function (measureId) {
        List.collapseItem();
        var archivedRecord = List.getUnarchivedRecordBy(measureId);
        List.dataList = LMEPG.Func.array.remove(archivedRecord);
        List.dataList.remove(archivedRecord);
                ...
                LMEPG.ajax.postAPI("DeviceCheck/archiveInspectRecord", postData, function (data) {
                    ...
                    if (data.result === 0) {
                        ...
                        LMEPG.UI.showToast("归档成功", 1.5, function () {
                            ...
                            List.deleteAndUpdateUI(record.measure_id);
                        });
                    } else {
        ...
        function _deleteInner() {
            ...
            LMEPG.ajax.postAPI("DeviceCheck/deleteInspectRecord", {measureId: measureId}, function (data) {
                ...
                if (data.result === 0) {
                    ...
                    List.deleteAndUpdateUI(measureId);
                } else {
...
var InspectRecordArchive = {
    ...
    initData: function () {
        ...
        // 保持并自动展开指定列表项
        if (RenderParam.actionType == 1 && !LMEPG.Func.isEmpty(lastFocusItemsId) && List.dataList.length > 0) {
