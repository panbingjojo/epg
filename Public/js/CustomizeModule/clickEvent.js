(function (w) {
    ...
    function getPositionData(position) {
        var listP = [];
        var dataList = RenderParam.moduleConfig.entry_list;

        for (var key in dataList) {
            listP.push(dataList[key]);
        }

        for (var i = 0; i < listP.length; i++) {
            if (listP[i].position == position) {
                return listP[i];
            }
        }
    ...
    function clickSwitch(item) {
        switch (parseInt(item.entry_type)) {
            ...
            case HomeEntryType.ACTIVITYS:
                PageJump.jumpActivityPage(JSON.parse(item.parameters)[0].param, btn);
                break;

            case HomeEntryType.HEALTH_VIDEO_BY_TYPES: // 更多视频
                PageJump.jumpMoreVideos(item);
                break;
    ...
    var PageJump = {
        ...
        jumpCommon: function (routerId) {
            LMEPG.Intent.jump(LMEPG.Intent.createIntent(routerId), PageEvent.setCurrentPage());
        },
        ...
        /**跳转 - 播放器*/
        jumpPlayVideo: function (itemData,other) {
            ...
            if(other){
                objPlayer.setParam('modelType', other);
            }
        ...
        jumpHoldPage: function () {
            var objHome = PageEvent.setCurrentPage();
            var objHold = LMEPG.Intent.createIntent('hold');0
