...
var Page = {
    ...
    jumpBuyVip: function (remark, videoInfo) {
        if (typeof videoInfo == 'object') {
            videoInfo.videoUrl = encodeURIComponent(videoInfo.videoUrl);
        }
...
var Player = {
    ...
    init: function () {
        ...
        if (RenderParam.userType == 2 && RenderParam.isVip != 1) {
            Show('tips');
        }
    ...
    getTimesArray: function () {
        ...
        switch (RenderParam.carrierId) {
            case '520092'://贵州电信电信EPG
                LMEPG.STBUtil.getSTBModel();
                duration = LMEPG.mp.getMediaDuration();
                switch (LMEPG.STBUtil.getSTBModel()) {
                    case 'B860AV1.1-T2':
                        // 该盒子需要用自己计算的时长，播放器时长会一直增加。
                        currentPlayTime = Player.Progress.currentDurtion;
                        break;
                    default:
    ...
    /**
     * 显示/隐藏 支付确定按钮
     * @param show true | false  显示/隐藏
     */
    setVisibilityForPayBtnUI: function (show) {
        if (show) {
            if (RenderParam.userType == 2 && RenderParam.isVip != 1) {
                Show('tips');
            }
        ...
        switch (RenderParam.carrierId) {
            ...
            case '520092':
                setTimeout(function () {
                    //LMEPG.mp.initPlayerMode1();
                    //LMEPG.mp.playOfFullscreen(RenderParam.videoUrl);
                    Player.videoCodeConvert520092(RenderParam.videoUrl);
                }, 500);
                break;
            default:
                setTimeout(function () {
    ...
    /** 贵州电信外部code转内部code接口 */
    videoCodeConvert520092:function (videoCode){
        var playerUrl = LMEPG.STBUtil.getEPGDomain();
        playerUrl = playerUrl.substring(0,playerUrl.indexOf('function'));
        LMEPG.Log.info('videoCodeConvert playerUrl！'+playerUrl);
        playerUrl = playerUrl+'frame1163/sdk_getinnercode.jsp?contenttype=1&contentcode='+videoCode;
        LMEPG.Log.info('videoCodeConvert playerUrl videoCode！'+playerUrl);
        LMEPG.ajax.GETAPI(playerUrl,function (data) {
            LMEPG.Log.info('videoCodeConvert 获取播放串！'+JSON.stringify(data));
            data = typeof data == 'object'?data:JSON.parse(data);
            if(data.returncode == '0'){
                Player.playerInterface520092(data);
            }else{
                LMEPG.UI.showToast('videoCodeConvert 获取播放串失败！'+data.errormsg);
                LMEPG.Log.info('videoCodeConvert 获取播放串失败！'+data.errormsg);
            }
        });
    },
    /** 外部code转内部code接口 */
    playerInterface520092:function (videoInfo){
        var playerUrl = LMEPG.STBUtil.getEPGDomain();
        playerUrl = playerUrl.substring(0,playerUrl.indexOf('function'));
        LMEPG.Log.info('playerInterface videoCodeConvert playerUrl！'+playerUrl);
        playerUrl = playerUrl+'frame1163/sdk_getmultidefinitionurl.jsp?programcode='+videoInfo.programcode+'&programtype=1';
        LMEPG.Log.info('playerInterface videoCodeConvert playerUrl videoCode！'+playerUrl);
        LMEPG.ajax.GETAPI(playerUrl,function (data) {
            LMEPG.Log.info('playerInterface videoCodeConvert 获取播放串！'+JSON.stringify(data));
            data = typeof data == 'object'?data:JSON.parse(data);
            if(data.returncode == '0'){
                factUrl = data.data[0].playurl;
                LMEPG.Log.info('videoCodeConvert playurl！'+factUrl);
                LMEPG.mp.initPlayer();
                LMEPG.mp.playOfFullscreen(factUrl);
            }else{
                LMEPG.UI.showToast('playerInterface videoCodeConvert 获取播放串失败！'+data.errormsg);
                LMEPG.Log.info('playerInterface videoCodeConvert 获取播放串失败！'+data.errormsg);
            }
        });
    },
...
Player.Progress = {
    ...
    start: function () {
        ...
        setTimeout(function () {
            ...
            window.mpTimer = setInterval(function () {
                debug2('Player.Progress.currentDurtion:' + Player.Progress.currentDurtion);
                if (LMEPG.mp.getPlayerState() === LMEPG.mp.State.PLAY && Player.Progress.currentDurtion < Player.Progress.videoDuration) {
                    //自己计算时长
                    Player.Progress.currentDurtion = eval(Player.Progress.currentDurtion + 1);
                }
                Player.Progress.onProgress();
                // console.log("Player.js-----isVip: " + RenderParam.isVip + ", userType: " + RenderParam.userType);
                // 检查免费时长 ，如果用户已经是vip，则不判断（isVip：1-是vip）
                if (RenderParam.isVip != 1 && LMEPG.mp.getPlayerState() !== LMEPG.mp.State.PAUSE) {
        ...
        if (this.currentSeekSecond >= 0) {
            switch (RenderParam.carrierId) {
                default:
                    ...
                    Player.Progress.currentDurtion = this.currentSeekSecond;
                    break;
            }
            this.resetSeekState();
        }
    ...
    beforeMoveIndicator: function (dir) {
        ...
        if (dir === C.Dir.LEFT || dir === C.Dir.RIGHT) {
            ...
            if (RenderParam.isVip != 1) {
                if (Player.Progress.checkFreeTime()) {
                    return false;
                }
            }
            ...
            this.currentSeekSecond = parseFloat(this.currentSeekSecond < 0 ? Player.Progress.currentDurtion : this.currentSeekSecond);//当前seek时间
    ...
    checkFreeTime: function () {
        ...
        if (parseInt(RenderParam.userType) == 2) {
            switch (LMEPG.STBUtil.getSTBModel()) {
                case 'B860AV1.1-T2':
                    // 该盒子需要用自己计算的时长，播放器时长会一直增加。
                    this.currentPlayTimes = Player.Progress.currentDurtion;
                    break;
                default:
    ...
    moveVolumeProgressBar: function (volumeValue) {
        ...
        G('volume_ball').style.left = (offset - 10) + 'px';//不显示球状拖动条
...
var PlayerKeyEventManager = {
    ...
    onBack: function () {
        if (isDetainPageShowing) {  // 退出播放器页面
            ...
            Player.setVisibilityForPayBtnUI(false);
    ...
    onClick: function (btn) {
        switch (btn.id) {
            ...
            case 'focus-2-1':
                ...
                Player.setVisibilityForPayBtnUI(true);
                break;
            case 'focus-2-2':
...
function _init_buttons() {
    buttons = [
        // 0: focus-1-1
        {
            ...
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Player/V11/bg_border.png',
