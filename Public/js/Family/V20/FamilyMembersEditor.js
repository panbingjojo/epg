        ...
        try {
            ...
            if (data instanceof Object) {
                if (data.result == 0) {
                    ...
                    LMEPG.UI.showToastV2("修改成功！", 1.5, function () {
                        onBack();
                    });
                } else {
                    LMEPG.UI.showToastV2("修改失败！:" + data.result);
                }
            } else {
                LMEPG.UI.showToastV2("修改失败！");
            }
        } catch (e) {
            LMEPG.UI.showToastV2("修改异常！");
        }
    }, function () {
        LMEPG.UI.dismissWaitingDialog();
        LMEPG.UI.showToastV2("修改异常！");
            ...
            try {
                ...
                if (data instanceof Object) {
                    if (data.result == 0) {
                        // 延迟处理，避免showToastV2未渲染出背景图仅显示出提示文本就执行onBack了。
                        LMEPG.KeyEventManager.setAllowFlag(false);
                        LMEPG.UI.showToastV2("删除成功！", 1.5, function () {
                            onBack();
                        });
                    } else {
                        LMEPG.UI.showToastV2("删除失败！" + data.result);
                    }
                } else {
                    LMEPG.UI.showToastV2("删除失败！");
                }

            } catch (e) {
                LMEPG.UI.showToastV2("删除异常！");
            }
        }, function () {
            LMEPG.UI.dismissWaitingDialog();
            LMEPG.UI.showToastV2("删除异常！");
...
var PageStart = {
    ...
    onBtnClick: function (btn) {
        if (btn.id === "parameter_5") {
            if(PageStart.isDel){
                LMEPG.UI.commonDialog.showV2('确定删除该成员吗？', ['确定', '取消'], function (btnId) {
                    if (btnId == 0) {
                        PageStart.delMember()
                    }
                });
    ...
    onBeforeMoveChange: function (direction, current) { //翻页
        switch (direction) {
            ...
            case "down":
                ...
                } else if (current.id == "parameter_5") {
                    PageStart.isDel = !PageStart.isDel;
                    G("parameter_5").innerHTML = PageStart.isDel ? "删除" : "提交";
                    G("parameter_5_next").innerHTML = PageStart.isDel ? "提交" : "删除";
    ...
    delMember: function () {
        LMEPG.UI.showWaitingDialog();
        var postData = {
            'member_id':InitData.memberArray[0].member_id
        };
        LMEPG.ajax.postAPI('Family/delMember', postData, function (rsp) {
            LMEPG.UI.dismissWaitingDialog();
            try {
                ...
                if (data instanceof Object) {
                    if (data.result == 0) {
                        // 延迟处理，避免showToastV2未渲染出背景图仅显示出提示文本就执行onBack了。
                        LMEPG.KeyEventManager.setAllowFlag(false);
                        LMEPG.UI.showToastV2('删除成功！', undefined, function () {
                            onBack();
                        });
                    } else {
                        LMEPG.UI.showToastV2('删除失败！' + data.result);
