...
var ThirdParty = {
    ...
    getAndShowWeilaiTVAds: function (completedCallback) {
        if (isRunOnPC) { //保护兼容，在PC浏览器上可以正常继续调试
            ...
            // 展示开屏广告
            LMAndroid.JSCallAndroid.doGetADInfo(JSON.stringify({"ad_type": "open"}), function (jsonFromAndroid) {
                try {
                    LMEPG.Log.info("getAndShowWeilaiTVAds::获取广告信息", jsonFromAndroid);
                    ...
                    if (result.status == 1 && LMEPG.Func.isObject(result.adspaces)
                        ...
                        LMEPG.Log.info("getAndShowWeilaiTVAds", "failed: judge condition is not satisfied!");
                        LMEPG.call(completedCallback);
                    }
                } catch (e) {
                    LMEPG.Log.info("getAndShowWeilaiTVAds", "exception: " + e.toLocaleString());
                    LMEPG.call(completedCallback);
                }
            });
        }
        ...
        if (LMEPG.Func.isArray(adMaterialsArray) && adMaterialsArray.length > 0) {
            ...
            function __report_showing_ad_log() {
                ...
                if (is_exist(showingMaterial)) {
                    LMAndroid.JSCallAndroid.doReportADLog(JSON.stringify({
                        mid: adspaces_0.mid,
                        aid: adspaces_0.aid,
