...
// 特殊：跳转疫情模块时，直接进入数据页
LMSplashRouter.routeEpidemic = function () {
    var epidemicIntent = LMEPG.Intent.createIntent('area-data-prev');
...
LMSplashRouter.routeV13VideoSet = function (routeParams) {
    ...
    if (model.navPage){
        /** 跳转二级目录*/
        splashRoute450001.routeVideoSetNewV13Secondary(model.modelType,model.modelName
            ,model.navPage,model.keepNavFocusId,model.page,model.navIndex);
    }else {
        splashRoute450001.routeVideoSetNewV13(model.modelType,model.modelName);
    }
}

var splashRoute450001 = {
    /** 路由新视频集--二级目录 */
