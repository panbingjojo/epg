...
var CommunityHosp = {
    marquee: marquee(),
    pageName: 'community-index',
    hospitalId: '',
    hospitalImg: '',
    hospitalNavArray:['online-case','experts-introduce','health-education','blood-manage','community-doctor'],
    init: function () {
        this.getHospData();
        this.assignHospitalNav(RenderParam.pageIndex);
        this.createBtns();
    },
    assignHospitalNav:function (pageIndex) {
        var i =pageIndex;
        //最多加载4个导航图片
        var maxLenth =parseInt(i)+4;
        dom = G('hospital-nav');
        strHtml = '';
        for(i;i<maxLenth;i++){
            strHtml +='<img id="'+CommunityHosp.hospitalNavArray[i]+'" src="'+ g_appRootPath + '/Public/img/hd/CommunityHospital/'+CommunityHosp.hospitalNavArray[i]+'.png" alt="">'
        }
        dom.innerHTML = strHtml;

        if(pageIndex == 1){
            G(CommunityHosp.hospitalNavArray[3]).style.marginRight='27px';
            G(CommunityHosp.hospitalNavArray[4]).style.marginRight='0';
            Hide('next');
            Show('prev');
        }else{
            Hide('prev');
    ...
    turnList: function (key, btn) {
        ...
        if (key === 'left' && btn.id === _this.hospitalNavArray[1]) {
            RenderParam.pageIndex = 0;
            CommunityHosp.assignHospitalNav(RenderParam.pageIndex);

            LMEPG.BM.requestFocus(_this.hospitalNavArray[0])
            return false;
        }
        if (key === 'right' && btn.id === _this.hospitalNavArray[3]) {
            RenderParam.pageIndex =1;
            //传入起始导航模块下标
            CommunityHosp.assignHospitalNav( RenderParam.pageIndex);

            LMEPG.BM.requestFocus(_this.hospitalNavArray[4]);
            return false;
        }
    },
    ...
    currentPage: function () {
        ...
        obj.setParam('areaName', RenderParam.areaName);
    ...
    jumpPage: function (btn) {
        ...
        } else if(btn.id === 'community-doctor'){
            if((areaName == "midong")){
                LMEPG.UI.showToast('功能暂未开放，敬请期待！', 2);
                return;
            }
        } else {
            if(btn.id == 'blood-manage'){
    ...
    createBtns: function () {
        ...
        LMEPG.BM.init(LMEPG.Func.getLocationString('focusId') || 'especially-department', buttons, true);
