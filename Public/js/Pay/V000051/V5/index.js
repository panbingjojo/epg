...
} else if (RenderParam.firstPayFocusPosition == 1) {
    LMEPG.ButtonManager.init('pay_item_1', buttons, '', true);
} else {
...
function onBack() {
    ...
    var judge2 = getComputedStyle(G('tempToast'), null).display;
    if (judge == 'none') {
        if (judge2 == 'block') {
            G('tempToast').style.display = 'none';
            LMEPG.BM.requestFocus(toastBtn);
        } else {
            LMEPG.Intent.back();
        }
    } else {
...
//                        }
//                    },'gid_common_button_0');
                toastBtn = LMEPG.BM.getCurrentButton().id;
                toastPay(btn);
...
function toastPay(btn) {
    payType = btn;
    G('tempToast').style.display = 'block';
    if (btn.cIndex == 0) {
        G('price-info').innerHTML = '-' + RenderParam.orderItems[btn.cIndex].price / 100 + '元包月';
    } else {
        G('price-info').innerHTML = '-' + RenderParam.orderItems[btn.cIndex].price / 100 + '元30天';
    }
    G('price-head').innerHTML = '￥' + RenderParam.orderItems[btn.cIndex].price / 100;
    G('price-right').innerHTML = '海量健康资讯，足不出户访名医，39健康，您的家庭健康顾问!';
    G('price-center').innerHTML = '￥' + RenderParam.orderItems[btn.cIndex].price / 100;
    LMEPG.BM.requestFocus('tempToast');
//        alert(LMEPG.BM.getCurrentButton().id);
}
...
function jumpOrderToast(id) {
    ...
    if (id == 'pay_item_0') {
        ...
        G('product_price').innerHTML = '<img src="__ROOT__/Public/img/' + RenderParam.platformType + '/Pay/V000051/V4/29_yuan.png" />'; // 资费信息
    }
