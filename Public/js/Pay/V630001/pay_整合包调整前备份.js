...
function onBack() {
    if (G("second-order-toast").style.display === "block") {
        LMEPG.BM.requestFocus(Pay.lastProductPkgFocusedId);
        Hide("second-order-toast");
    } else {
        LMEPG.Intent.back();
    }
}
...
var Pay = {
    ...
    lastProductPkgFocusedId: isSinglePay ? "cancel" : "btn-product-1",       // 展示页1 - 最后选中的产品包焦点元素id
    ...
    initData: function () {
        ...
        if (!isSinglePay) {
            for (var i = 0, len = RenderParam.orderItems.length; i < len; i++) {
                buttons.push({
                    id: "btn-product-" + (i + 1),
                    name: "套餐包-" + (i + 1),
                    type: "img",
                    nextFocusLeft: "btn-product-" + (i === 0 ? len : i),
                    nextFocusRight: "btn-product-" + (i >= len - 1 ? 1 : i + 2),
                    nextFocusDown: "cancel",
                    backgroundImage: imgUrl + "product" + (i + 1) + "_unfocused.png",
                    focusImage: imgUrl + "product" + (i + 1) + "_focused.png",
                    click: Pay.onProductPkgClicked,
                    focusChange: Pay.onProductPkgFocusChanged,
                    cIndex: i, //订购项下标索引（0开始）
                    cPrice: RenderParam.orderItems[i].price, //绑定配置的定价（分），用于后面订购匹配查找
                });
            }
        }
    },
    ...
    init: function () {
        ...
        LMEPG.BM.init(Pay.lastProductPkgFocusedId, buttons, "", true);
    },
    ...
    findMatchAuthProduct: function (authProductObj) {
        ...
        for (var i = 0, lenI = configOrderItems.length; i < lenI; i++) {
            ...
            if (item.price/*maybe string*/ == selectUIPrice) {
    ...
    _ADVPay: function () {
        ...
        if (payParam.payOrderItem == null || payParam.payAuthProduct == null) {
            LMEPG.UI.showToast("该套餐包申请中，敬请关注...");
            return;
        }
...
var SecondToast = {
    ...
    randomNumber: function () {
        ...
        G("verify").innerHTML = renderUIExpr;
    },
    ...
    toSubmit: function () {
        ...
        if (inputVal.trim() === "") {
            ...
            if (inputVal/*string*/ == VCAlgorithmTool.calRandomResult().displayResult) {
                console.log("验证码正确");
                Pay._ADVPay(); // 执行订购操作
            } else {
