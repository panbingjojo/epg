var buttons = [];   // 定义全局按钮
var imgUrl2 = ROOT + "/Public/img/hd/Pay/V630001/V2";
var imgUrl = ROOT + "/Public/img/hd/Pay/V630001/V1";
var formula = "";
var left_select = "";
var leftBg = "";
var img=[];
var topPosition = [0, -380, -680];
var isShow59Y = false;
...
var Pay = {
    ...
    initbuttons:function(){
        //添加爱家全站会员
        if(isShow59Y){
            G('love-home-59').style.display = 'inline-block';
            for(var i = 0; i < buttons.length; i++) {
                if (buttons[i].id=='btn-product-1'){
                    buttons[7].nextFocusDown = buttons[i].nextFocusDown;
                    buttons[i].nextFocusDown = buttons[7].id;
                }
                if (buttons[i].id=='btn-product-2'){
                    buttons[7].nextFocusUp = buttons[i].nextFocusUp;
                    buttons[i].nextFocusUp = buttons[7].id;
                }

                if (buttons[i].id=='btn-product-7'){
                    buttons[i].nextFocusDown = "";
                }
                if (buttons[i].id=='btn-product-8'){
                    buttons[i].nextFocusRight = "tab-1";
                }
            }
        }else{
            for(var i = 0; i < buttons.length; i++) {
        ...
        if(isNuanXin){
            for(var i = 0; i < buttons.length; i++) {
                if (buttons[i].id=='btn-product-1'){
                    buttons[i].nextFocusUp = "btn-product-lv-3";
                    buttons[i].nextFocusDown = "btn-product-2";
                }
                if (buttons[i].id=='btn-product-2'){
                    buttons[i].nextFocusUp = "btn-product-1";
                }

                if (buttons[i].id=='btn-product-lv-1'){
                    buttons[i].nextFocusUp = "btn-product-lv-4";
                    buttons[i].nextFocusDown = "btn-product-lv-3";
                }
                if (buttons[i].id=='btn-product-lv-2'){
                    buttons[i].nextFocusUp = "";
                    buttons[i].nextFocusDown = "btn-product-lv-4";
                }
                if (buttons[i].id=='btn-product-lv-3'){
                    buttons[i].nextFocusUp = "btn-product-lv-1";
                    buttons[i].nextFocusDown = "btn-product-1";
                    buttons[i].nextFocusRight = "tab-1";
                }
                if (buttons[i].id=='btn-product-lv-4'){
                    buttons[i].nextFocusUp = "btn-product-lv-2";
                    buttons[i].nextFocusDown = "btn-product-lv-1";
                }
            }
        }
    },
    ...
    initData: function () {
        ...
        for (var i = 0, len = 8; i < len; i++) {
            buttons.push({
                ...
                type: "img",
                nextFocusLeft: "",
                nextFocusRight:  i+1 <= 4? 'tab-1' : "tab-0",
                nextFocusUp: i==0?"btn-product-lv-2":"btn-product-" + i,
                nextFocusDown: "btn-product-" + (i + 2),
                backgroundImage: imgUrl2 + '/pay_' + (i + 1) + '.png',
                focusImage: imgUrl2 + '/pay_' + (i + 1) + '_f.png',
                selectedImage: imgUrl2 + '/pay_' + (i + 1) + '_select.png',
        ...
        for (var i = 0, len = 4; i < len; i++) {
            buttons.push({
                id: "btn-product-lv-" + (i + 1),
                name: "套餐包-lv-" + (i + 1),
                type: "img",
                nextFocusLeft: "",
                nextFocusRight:  i==0 ? 'tab-1' : "tab-0",
                nextFocusUp: "btn-product-lv-" + i,
                nextFocusDown: i==1?"btn-product-1":"btn-product-lv-" + (i + 2),
                backgroundImage: imgUrl2 + '/pay_lv_' + (i + 1) + '.png',
                focusImage: imgUrl2 + '/pay_lv_' + (i + 1) + '_f.png',
                selectedImage: imgUrl2 + '/pay_lv_' + (i + 1) + '_select.png',
                click: Pay.onProductPkgClicked,
                focusChange: Pay.onProductPkgFocusChanged,
                beforeMoveChange: Pay.onProductPkgBeforeMoveChanged,
                cIndex: i, //订购项下标索引（0开始）
        ...
        while (FOCUS_COUNT--) {
            buttons.push({
                ...
                type: 'img',
                nextFocusUp: '',
                nextFocusLeft: 'tab-' + (FOCUS_COUNT - 1),
                nextFocusRight: 'tab-' + (FOCUS_COUNT + 1),
                backgroundImage: imgUrl + '/tab/tab_' + (FOCUS_COUNT + 1) + '.png',
                focusImage: imgUrl + '/tab/tab_' + (FOCUS_COUNT + 1) + '_f.png',
                selectedImage: imgUrl + '/tab/tab_' + (FOCUS_COUNT + 1) + '_select.png',
                focusChange: Pay.tabFocus,
                beforeMoveChange: Pay.onTabBeforeMoveChanged,
                groupId: 'tab',
            });
        }
        ...
        buttons.push({
            ...
            backgroundImage: imgUrl2 + '/confirm.png',
            focusImage: imgUrl2 + '/confirm_f.png',
            click: Pay.onClicked,
            beforeMoveChange: Pay.onProductPkgBeforeMoveChanged,
        });
        ...
        buttons.push({
            ...
            nextFocusUp: "tab-1",
            nextFocusDown: "btn-agreement",
            backgroundImage: imgUrl2 + '/cancel.png',
            focusImage: imgUrl2 + '/cancel_f.png',
            click: onBack,
            beforeMoveChange: Pay.onProductPkgBeforeMoveChanged,
        });
        ...
        buttons.push({
            ...
            backgroundImage: imgUrl2 + '/agreement.png',
            focusImage: imgUrl2 + '/agreement_f.png',
            click: Pay.onClicked,
            focusChange: Pay.onProductPkgFocusChanged,
            beforeMoveChange: Pay.onProductPkgBeforeMoveChanged,
        });
    ...
    onProductPkgFocusChanged: function (btn, hasFocus) {
        if (hasFocus) {
            if (btn.id.search("btn-product-5") != -1||
                btn.id.search("btn-product-6") != -1||
                btn.id.search("btn-product-7") != -1||
                btn.id.search("btn-product-lv-2") != -1 ||
                btn.id.search("btn-product-lv-4") != -1) {
                G('tab-0').style.display = 'inline-block';
                G('tab-1').style.display = 'inline-block';
                G('tab-1').style['padding-left']="15px";
                Show("tab-0-link");
                Hide("tab-1-link");
                G("tab-1").src=LMEPG.BM.getButtonById("tab-1").backgroundImage;
                G("tab-0").src=LMEPG.BM.getButtonById("tab-0").selectedImage;
                //Pay._ADVQRPay(btn.id);
                if(btn.id == "btn-product-5"){
                    G("code").src = img[0];
                }
                if(btn.id == "btn-product-6"){
                    G("code").src = img[1];
                }
                if(btn.id == "btn-product-7"){
                    G("code").src = img[2];
                }
                if(btn.id == "btn-product-lv-2"){
                    G("code").src = img[3];
                }
                if(btn.id == "btn-product-lv-4"){
                    G("code").src = img[4];
                }
            }else{
                Hide("tab-0-link");
                Show("tab-1-link");
                G('tab-1').style['padding-left']="0px";
            }


            if (btn.id.search("btn-product-lv-2") != -1 ||
                btn.id.search("btn-product-lv-4") != -1) {
                Hide("tab-1");
                Hide("tab-1-link");
                Show("tab-0");
            ...
            if (btn.id.search("btn-product-4") != -1 ||
                btn.id.search("btn-product-lv-1") != -1 ||
                btn.id.search("btn-product-lv-3") != -1) {
                Hide("tab-0");
                Hide("tab-0-link");
                Show("tab-1");
                Show("tab-1-link");
                G("tab-1").src=LMEPG.BM.getButtonById("tab-1").selectedImage;
            }
    ...
    tabFocus: function (btn, hasFocus) {
        if (hasFocus) {
            if (left_select.search("product") != -1) {
                G(left_select).src = leftBg;
            }
            if(btn.id.substring(0,4)=="tab-"){
    ...
    onClicked: function (btn) {
        if (btn.id == "btn-agreement") {
            ...
            if (count == count_p || left_select == "btn-product-1" || true) {
                Pay._ADVPay(); // 执行订购操作
            } else {
    ...
    onProductPkgBeforeMoveChanged: function (dir, current) {
        if (dir == 'up' || dir == 'down') {
            ...
            if (current.id == "btn-product-lv-1") {//btn-product-1
                dir == 'down' ? G("scroll").style.top = topPosition[2] + "px" : "";
            }


            if(isShow59Y){
                if (current.id == "btn-product-8") {
                    dir == 'down' ?"" : G("scroll").style.top = topPosition[0] + "px";
                }

                if (current.id == "btn-product-3") {
                    dir == 'down' ? G("scroll").style.top = topPosition[2]*2+135 + "px" : G("scroll").style.top = topPosition[2] + "px";
                }
            }else {
                if (current.id == "btn-product-2") {
                    dir == 'down' ? G("scroll").style.top = topPosition[2]*2+35 + "px" : "";
                }

                if (current.id == "btn-product-3" ) {
                    dir == 'up' ? G("scroll").style.top = topPosition[2] + "px" : "";
                }

                if (current.id == "btn-product-lv-3" ) {//btn-product-lv-3
                    dir == 'up' ? G("scroll").style.top = topPosition[0] + "px" : "";
                }
            }

            if (current.id == "btn-product-4" && dir == 'up' ) {
                //G("scroll").style.top = topPosition[0] + "px";
            }
        }
        ...
        if (dir == 'right') {
            if (current.id.search("product") != -1) {
                left_select = current.id;
                leftBg = current.selectedImage;
                // G(left_select).src=current.selectedImage
    ...
    onTabBeforeMoveChanged: function (dir, current) {
        if (dir == 'left') {
            if (current.id == "tab-0"
                || left_select == "btn-product-lv-1"
                || left_select == "btn-product-8"
                || left_select == "btn-product-4"
                || left_select == "btn-product-3"
                || left_select == "btn-product-2"
                || left_select == "btn-product-1") {
                LMEPG.BM.requestFocus(left_select);
                LMEPG.ButtonManager.setSelected(current.id, true)
                return false;
            }
            ...
            if (current.id == "tab-1") {
                LMEPG.BM.requestFocus("btn-back");
                LMEPG.ButtonManager.setSelected(current.id, true);   // 设置导航栏菜单的选中项
                return false;
            }
        }
    ...
    setADVQRPay: function () {
        Pay._ADVQRPay("btn-product-lv-2");
        setTimeout(this.setADVQRPay, 1000*20);
    },
    ...
    init: function () {
        ...
        //Pay.initUIFirst();
        var focusid="";
        if(date.isDuringDate('2020/12/21', '2021/01/22')){
            focusid = "btn-product-2";
        }else{
            focusid = "btn-product-lv-2";
        }
        //left_select=focusid;
        LMEPG.BM.init(focusid, buttons, "", true);
        //LMEPG.BM.init("btn-product-1", buttons, "", true);
        //Pay._ADVQRPay("btn-product-1");//二维码支付
        Hide("tab-0");
        Hide("tab-0-link");
        Show("tab-1");
        Show("tab-1-link");
     ...
     */
    findMatchAuthProduct: function (authProductObj,focus_id) {
        ...
        var priceRenderMapping = {
            ...
            5: 5,    //爱家健康会员测试
            6: 7900,    //电视家庭医生包季79元
            7: 15800,    //电视家庭医生包半年158元
            8: 29900,    //电视家庭医生包年299元
            9: 5900,    //爱家全站包月59元
            10: 3900,    //爱家全站包月39元
            11: 19900,    //爱家全站包月39元
            12: 3500,    //暖心包35元
            13: 36000    //爱家全站特惠购包年360元
        };
        ...
        if (left_select == "btn-product-1") {
            selectProductId = "8802000810";
            selectUIPrice = 2000;
        } else if (left_select == "btn-product-2") {
            selectProductId = "8802000018";
            selectUIPrice = 2900;
        } else if (left_select == "btn-product-3") {
            selectProductId = "8802000629";
            selectUIPrice = 800;
        } else if (left_select == "btn-product-4") {
            selectProductId = "8802000630";
            selectUIPrice = 1500;
        }else if (left_select == "btn-product-5") {
            selectProductId = "8802001179";
            selectUIPrice = 7900;
        }else if (left_select == "btn-product-6") {
            selectProductId = "8802001180";
            selectUIPrice = 15800;
        }else if (left_select == "btn-product-7") {
            selectProductId = "8802001181";
            selectUIPrice = 29900;
        }else if (left_select == "btn-product-8") {
            selectProductId = "8802001312";
            selectUIPrice = 5900;
        }else if (left_select == "btn-product-lv-1") {
            selectProductId = "8802001314";
            selectUIPrice = 3900;
        }else if (left_select == "btn-product-lv-2") {
            selectProductId = "8802001370";
            selectUIPrice = 19900;
        }else if (left_select == "btn-product-lv-3") {
            selectProductId = "8802001499";
            selectUIPrice = 3500;
        }else if (left_select == "btn-product-lv-4") {
            selectProductId = "8802001550";
            selectUIPrice = 36000;
        }

        if(focus_id == "btn-product-5"){
            selectProductId = "8802001179";
            selectUIPrice = 7900;
        }else if (focus_id == "btn-product-6") {
            selectProductId = "8802001180";
            selectUIPrice = 15800;
        }else if (focus_id == "btn-product-7") {
            selectProductId = "8802001181";
            selectUIPrice = 29900;
        }else if (focus_id == "btn-product-lv-2") {
            selectProductId = "8802001370";
            selectUIPrice = 19900;
        }else if (focus_id == "btn-product-lv-4") {
            selectProductId = "8802001550";
            selectUIPrice = 36000;
        }
     ...
     * 二维码下单接口
     */
    _ADVQRPay: function (select_id) {
        //left_select = select_id;
        // 查找到匹配对应的“我方配置项”及“对应局方订购包”，我方展示（及后台配置项）顺序与局方订购产品包顺序一致了！
        //debugger
        var payParam = Pay.findMatchAuthProduct(RenderParam.authProductInfo,select_id);
        if (payParam.payOrderItem == null || payParam.payAuthProduct == null) {
            // LMEPG.UI.showToast('<div id="g_title" class=" g_title">该套餐包申请中，敬请关注...</div>');
            return;
        }
        ...
        //LMEPG.UI.showWaitingDialog("请等待...");
        LMEPG.ajax.postAPI("Pay/ADVPay", postData, function (data) {
            if (data.result == "0") {
                // 下单成功
                //Pay.isClickPaying = true;
                ...
                LMEPG.UI.dismissWaitingDialog();
                Pay.payingData.payParamInfo.qrCodeImg = Pay.payingData.payParamInfo.qrCodeImg.replace(/\ +/g, "");
                payimg = "data:image/png;base64," + Pay.payingData.payParamInfo.qrCodeImg;
                //G("code").src = payimg;
                if(select_id == "btn-product-5"){
                    img[0]=payimg;
                    Pay._ADVQRPay("btn-product-6");
                }
                if(select_id == "btn-product-6"){
                    img[1]=payimg;
                    Pay._ADVQRPay("btn-product-7");
                }
                if(select_id == "btn-product-7"){
                    img[2]=payimg;
                }
                if(select_id == "btn-product-lv-2"){
                    img[3]=payimg;
                    if(LMEPG.BM.getCurrentButton().id === "btn-product-lv-2"){
                        Hide("tab-1");
                        Hide("tab-1-link");
                        Show("tab-0");
                        Show("tab-0-link");
                    ...
                    Pay._ADVQRPay("btn-product-lv-4");
                }
                if(select_id == "btn-product-lv-4"){
                    img[4]=payimg;
                    Pay._ADVQRPay("btn-product-5");
                }
            } else {
                // 下单失败
                LMEPG.UI.dismissWaitingDialog();
    ...
    _AuthenTic: function () {
        var postData = {};
        LMEPG.ajax.postAPI("Pay/Authentication", postData, function (data) {
            if (data.result == "0" && data.isVIP) {
                LMEPG.UI.showToast('<div id="g_title" class=" g_title">支付成功</div>');
                uploadOrderData();
            } else {
                setTimeout(Pay._AuthenTic, 1000);
            }
        });
    },
...
function uploadOrderData() {
    var payData = {
        apkInfo: encodeURIComponent(JSON.stringify(RenderParam.apkInfo)),
        orderItem: JSON.stringify(Pay.payingData.orderItem),
        productInfo: JSON.stringify(Pay.payingData.authProduct),
        payParamInfo: JSON.stringify(Pay.payingData.payParamInfo),
        orderInfo: JSON.stringify(Pay.payingData.payParamInfo),
    };
    LMEPG.ajax.postAPI("Pay/upLoadAuthentication", payData, function (data) {
        onBack();
    });
}
...
var SecondToast = {
    ...
    init: function () {
        ...
        LMEPG.ButtonManager.setSelected("tab-0", true)
    },
