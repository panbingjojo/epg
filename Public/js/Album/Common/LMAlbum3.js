var LMAlbum = {
    ...
    initRegional: function (albumAlias) {
        ...
        var recommendRouteHomeCarriers = ['000709','410092','420092','430002','440001','450092','220001','640001','000005','150002','370092','430012','450004','01230001','450001', '450094'];
    ...
    goBack: function () {
        if(RenderParam.lmcid == '610092'||RenderParam.lmcid == "10000051" || RenderParam.lmcid == "10000006"){
            LMEPG.Intent.back();
            return;
        }
        if (LMAlbum.dialogId) {
    ...
    renderVideoList: function () {
        var videoListHtml = '';
        for (var videoIndex = 0; videoIndex < LMAlbum.showVideoList.length; videoIndex++) {
    ...
    beforeVideoMoveChange: function (direction, btn) {
        if (LMAlbum.videoLayout == LMAlbum.LayoutDirection.Horizontal) {
            switch (direction) {
                ...
                case "down":
                    if (btn.id == "video-1") {
                        LMEPG.BM.requestFocus("video-3");
                        return false;
                    } else if (btn.id == "video-2") {
                        LMEPG.BM.requestFocus("video-4");
                        return false;
                    }
                    break;
                case "up":
                    if (btn.id == "video-3") {
                        LMEPG.BM.requestFocus("video-1");
                        return false;
                    } else if (btn.id == "video-4") {
                        LMEPG.BM.requestFocus("video-2");
                        return false;
                    }
                    break;
            }
    ...
    toggleNextVideo: function (btn) {
        ...
        if (btn.id == 'video-2' ||  btn.id == 'video-4') {
            if(lastVideoIndex > (videoIndex + ( btn.id == 'video-2' ? 2 : 1)))
                return LMAlbum.renderNextVideo(btn, videoIndex);
            else
                return false;
        }
    },
    ...
    togglePrevVideo: function (btn) {
        if (btn.id == 'video-1' || btn.id == 'video-3') {
            var videoIndex = parseInt(G(btn.id).getAttribute('data-v'));
            return LMAlbum.renderPrevVideo(btn, videoIndex);
        }
    },

    renderNextVideo: function (btn, videoIndex) {
        var lastVideoIndex = LMAlbum.videoList.length - 1;
        if ((videoIndex == lastVideoIndex) || ((videoIndex + 2) == lastVideoIndex)) {
            LMEPG.ButtonManager.requestFocus(LMAlbum.videoMoveConfig.next);
            return false;
        } else {
            // 最后显示的元素
            var videNum = (lastVideoIndex - videoIndex) >= 4 ? 4 : lastVideoIndex - videoIndex;
            if(btn.id == 'video-2')
            {
                videNum = (lastVideoIndex - videoIndex - 2) >= 4 ? 4 : lastVideoIndex - videoIndex - 2;
                videoIndex += 2;
            }
            ...
            for(var i = 1; i <= videNum; i++)
                LMAlbum.showVideoList.push(LMAlbum.videoList[videoIndex + i]);
            LMAlbum.renderVideoList();
            LMEPG.ButtonManager.requestFocus('video-1');
            LMAlbum.firstVideoIndex++;
            var nextVideoIndex = btn.is == 'video-1' ? videoIndex+ + 3 : videoIndex + 1;
            if (nextVideoIndex + 3 >= (lastVideoIndex - 1)) {
                LMAlbum.showVideoArrow(LMAlbum.ArrowStatus.ShowPrev);
            } else {
                LMAlbum.showVideoArrow(LMAlbum.ArrowStatus.ShowAll);
            }
    ...
    renderPrevVideo: function (btn, videoIndex) {
        if (videoIndex == 0 || videoIndex == 2) {
            LMEPG.ButtonManager.requestFocus(LMAlbum.videoMoveConfig.prev);
            return false;
        } else {
            LMAlbum.showVideoList.length = 0;

            for(var i = 0; i < 4; i++) {
                if(btn.id == 'video-1')
                    LMAlbum.showVideoList.push(LMAlbum.videoList[videoIndex - 4 + i]);
                else
                    LMAlbum.showVideoList.push(LMAlbum.videoList[videoIndex - 6 + i]);
            }

            LMAlbum.renderVideoList();
            LMEPG.ButtonManager.requestFocus('video-1');
            LMAlbum.firstVideoIndex -= 4;
            if (videoIndex == 4 || videoIndex == 6) {
                LMAlbum.showVideoArrow(LMAlbum.ArrowStatus.ShowNext);
            } else {
                LMAlbum.showVideoArrow(LMAlbum.ArrowStatus.ShowAll);
            }
    ...
    commonBack: function () {
        ...
        if (RenderParam.lmcid == '12650092' || RenderParam.lmcid == '650092' || RenderParam.lmcid == '520092') {
