    ...
    exitAppHome: function () {
        switch (RenderParam.carrierId) {
            ...
            case '340092':// 安徽电信EPG
                ...
                break;
            case '220094'://吉林广电EPG
            case '10220094'://吉林广电魔方EPG
            case '220095'://吉林广电电信EPG
            case '10220095'://吉林电信魔方EPG
                try {
                    if(RenderParam.carrierId == '10220094'){
                        var url = 'http://139.215.92.20:10002/index.php?lmuf=3&lmsid&lmsl=hd-1&lmcid=220094&lmp=32&POSITIVE';
                    }else{
                        var url = 'http://10.128.3.146:10001/index.php?lmuf=0&lmsid=&lmsl=hd-1&lmcid=220095&lmp=32&POSITIVE';
                    }
                    if(true){
            ...
            case '210092': // 辽宁电信EPG
                ...
                break;
            case "10000051":
                if (RenderParam.areaCode == '216' && true) {
                    var goUrl = 'http://202.99.114.152:30214/index.php?lmuf=0&lmsid=&lmsl=hd-1&lmcid=000051&lmp=285&lmacid=216&userId='+
                    //var goUrl = 'http://202.99.114.28:25080/childVideo/topSkip.html?Entry=SDSEJKMF&carrierid=216&userId=' +
            ...
            case '410092':
                ...
                break;
            default:
                LMEPG.Log.info("hold.js---exitAppHome---isRunOnAndroid::" + RenderParam.isRunOnAndroid);
                if (RenderParam.isRunOnAndroid == 1) { // Android平台直接退出App
                    LMAndroid.JSCallAndroid.doExitApp();
                } else {                              // EPG平台根据返回链接进行跳转
                    LMEPG.Intent.back('IPTVPortal');
                }
                break;
        }
        ...
        switch (configInfo.entryType) {
            ...
            case '5':
                ...
                var videoInfo = {
                    ...
                    'type': configInfo.modelType || configInfo.model_type,
                    'userType': configInfo.userType || configInfo.user_type,
                    'freeSeconds': configInfo.freeSeconds  || configInfo.free_seconds,
                    'entryType': 1,
                    'entryTypeName': '挽留页',
            ...
            case '57':
                ...
                objDst = LMEPG.Intent.createIntent('channel');
                objDst.setParam('userId', RenderParam.userId);
                objDst.setParam('modeType', configInfo.entryType);     //56:中老年健康、57:宝贝天地
                objDst.setParam('modeTitle', configInfo.entryType == '56' ? '中老年健康' : '宝贝天地');
                objDst.setParam('inner', 1);
                break;
            case '58':
                ...
                return;
            case '43':
                var graphicId = configInfo.source_id;
...
var Hold = {
    ...
    onRecommendClick: function (btn) {
        ...
        var configInfo = G(btn.id).getAttribute("data_json");
    ...
    createList: function () {
        ...
        if (currentData.length > 0) {
            ...
            for (var i = 0; i < currentData.length; i++) {
                ...
                html += '<img id=' + btnId + ' class="recommend_img" ' + 'data_json=' + data_json + ' src="' + RenderParam.fsUrl + currentData[i].image_url + '"';
                html += '</img>';
            }
            html += '</div>';
    ...
    updateBackground: function () {
        var background = RenderParam.platformType == 'hd' ? RenderParam.fsUrl + RenderParam.tipsData[2].onfocus_image_url : RenderParam.fsUrl + RenderParam.tipsData[2].onblur_image_url;
    ...
    initButton: function () {
        ...
        var bgBackImg = RenderParam.fsUrl + RenderParam.tipsData[1].onfocus_image_url;
        var bgFBackImg = RenderParam.fsUrl + RenderParam.tipsData[1].onblur_image_url;
        var bgContinueImg = RenderParam.fsUrl + RenderParam.tipsData[0].onfocus_image_url;
        var bgFContinueImg = RenderParam.fsUrl + RenderParam.tipsData[0].onblur_image_url;
        ...
        buttons.push({
            ...
            name: '精彩继续',
            type: 'img',
            nextFocusLeft: RenderParam.carrierId === '450094' ? 'back' : '',
            nextFocusRight: RenderParam.carrierId === '450094' ? '' : 'back',
            nextFocusUp: '',
        ...
        buttons.push({
            ...
            name: '残忍离开',
            type: 'img',
            nextFocusLeft: RenderParam.carrierId === '450094' ? '' : 'continue',
            nextFocusRight: RenderParam.carrierId === '450094' ? 'continue' : '',
            nextFocusUp: '',
            nextFocusDown: RenderParam.dataList.length > 0 ? 'recommend_0' : '',
    ...
    init: function () {
        ...
        Hold.createList();
        Hold.initButton();
        Hold.initByCarrierIdId()
    },

    /**
     * 根据地区ID，初始化buttons
     */
    initByCarrierIdId: function () {
        if (RenderParam.carrierId == "10000051") {
            LMEPG.ButtonManager.init('recommend_0', buttons, '', true);
