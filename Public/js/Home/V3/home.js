...
var Page = {
     ...
     * 跳转 - 切换导航页
     */
    jumpHomeTab: function (tabName, classifyId, focusIndex) {
        var objCurrent = Page.getCurrentPage();
    ...
    jumpBuyVip: function (remark, videoInfo, singlePayItem) {
        if (typeof (videoInfo) !== 'undefined' && videoInfo !== '') {
            ...
            LMEPG.ajax.postAPI("Player/storeVideoInfo", postData, function (data) {
                Page.routerToOrder(remark, singlePayItem);
    ...
    routerToOrder: function (remark, singlePayItem) {
        var objHome = Page.getCurrentPage();
        objHome.setParam('userId', RenderParam.userId);
        objHome.setParam('classifyId', Home.classifyId);
...
var Play = {
        ...
        setTimeout(function () {
            ...
            if (RenderParam.platformType == 'hd') {
                ...
                switch (stbModel) {
                    ...
                    // case "B860AV1.1-T2":
                    //     playPosition = [parseInt(244), parseInt(100), parseInt(350), parseInt(200)];
                    //     break;
                }
            } else {
                // 标清
    ...
    onPlayEvent: function (keyCode) {
        if (LMEPG.mp.isEnd(keyCode) || LMEPG.mp.isError(keyCode)) {
            if (LMEPG.mp.isError(keyCode)) {
                // 备注：由于此款盒子“EC6108V9U_pub_gxgdx”（业务账号/正式服userId（iptv77114245775/2394554））播放小窗会频繁发送768的EVENT_MEDIA_ERROR信号，
                // 但又不影响视频正常播放。故暂时屏蔽掉对其EVENT_MEDIA_ERROR的终止处理，让其获得持续播放权。
                // ---------> Last modified by Songhui on 2019-7-5 #Bug-Fix-广西电信EPG盒子首页小窗播放闪烁循环播放。
                var stbModel = LMEPG.STBUtil.getSTBModel();
                if (LMEPG.Log) LMEPG.Log.error('[' + RenderParam.carrierId + '][V3/home.js]--[onPlayEvent(keyCode)]--->keyCode: ' + keyCode + ', occurred on stbModel--->' + stbModel);
                if (RenderParam.carrierId === '450092' && stbModel === 'EC6108V9U_pub_gxgdx') {
                    // DO NOTHING, RETURN DIRECTLY HERE...
                    return;
                }
            }
...
var Home = {
    ...
    Banner: {
        ...
        startTimer: function (position) {
            if (position == 1) {
                ...
                Home.Banner.setBannerInitCorner(Home.Banner.item[0].entryType, 1);
                ...
                Home.Banner.setBannerInitCorner(Home.Banner.item5[0].entryType, 5);
                this.timer5 = setInterval(Home.Banner.triggerTimer(5), this.MAX_TIMER_DURATION);
            }
        },
        //设置初始化角标
        setBannerInitCorner: function (entryType, position) {
            if (position == 1) {
                if (entryType == 5) {
                    Home.showCornerFree('recommend_parent_1', 1, Home.Banner.item[0]);
                } else {
                    if (G('corner_free_1')) {
                        G('recommend_parent_1').removeChild(G('corner_free_1'));//移除一号位置的免费角标
                    }
                }
            } else if (position == 5) {
                if (entryType == 5) {
                    Home.showCornerFree('recommend_parent_5', 5, Home.Banner.item5[0]);
                } else {
                    if (G('corner_free_5')) {
                        G('recommend_parent_5').removeChild(G('corner_free_5'));//移除一号位置的免费角标
                    }
                }
            }
        },
        //设置角标
        setBannerCorner: function (entryType, position) {
            if (position == 1) {
                if (entryType == '5') {
                    if (G('corner_free_1')) {
                        G('recommend_parent_1').removeChild(G('corner_free_1'));//移除一号位置的免费角标
                    }
                    Home.showCornerFree('recommend_parent_1', 1, Home.Banner.item[Home.Banner.bannerIndex]);
                    var corner = G('corner_free_' + 1);
                    var img = G('recommend_' + 1);
                    if (corner) {
                        if (img.offsetTop < 0) {
                ...
                if (entryType == '5') {
                    ...
                    Home.showCornerFree('recommend_parent_5', 5, Home.Banner.item5[Home.Banner.bannerIndex5]);
                    var corner = G('corner_free_' + 5);
                    var img = G('recommend_' + 5);
                    if (corner) {
                        if (img.offsetTop < 0) {
                            corner.style.top = img.offsetTop + 4 + 'px';
                            corner.style.left = img.offsetLeft + 4 + 'px';
        ...
        triggerTimer: function (position) {
            return function () {
                if (position == 1) {
                    ...
                    var entryType = Home.Banner.item[Home.Banner.bannerIndex].entryType;
                    Home.Banner.setBannerCorner(entryType, 1);
                } else if (position == 5) {
                    ...
                    var entryType = Home.Banner.item5[Home.Banner.bannerIndex5].entryType;
                    Home.Banner.setBannerCorner(entryType, 5);
                }
            }
        },
    ...
    onNavFocus: function (btn, hasFocus) {
        if (hasFocus == true) {
            var onfocusurl = G(btn.id).getAttribute('onfocusurl');
            G(btn.id).style.backgroundImage = 'url(' + onfocusurl + ')';
        } else {
    ...
    onNavClick: function (btn) {
        ...
        switch (btn.id) {
            case 'nav_0':
                break;
            case 'nav_1':
                Page.jumpHomeTab('homeTab1', '1');
                break;
            case 'nav_2':
                Page.jumpHomeTab('homeTab2', '2');
                break;
            case 'nav_3':
                Page.jumpHomeTab('homeTab3', '3');
                break;
            case 'nav_4':
                Page.jumpHomeTab('homeTab4', '4');
                break;
        }
    },
