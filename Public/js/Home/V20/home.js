...
var Page = {
    ...
    clickedButton: null, // 当前被点击的推荐位的按钮对象
    ...
    init: function () {
        ...
        if (RenderParam.areaCode === '207') {
            LMEPG.BM.requestFocus(this.focusId || "tab0-recommended-6");
        } else {
        ...
        LMEPG.BM.addButtons([
            {
                ...
                nextFocusUp: '',
                backgroundImage: "/Public/img/hd/Home/V16/Home/back.png",
                focusImage: "/Public/img/hd/Home/V16/Home/back-choose.png",
                click: function () {
                ...
                nextFocusRight: '',
                nextFocusUp: '',
                backgroundImage: "/Public/img/hd/Home/V16/Home/continue.png",
                focusImage: "/Public/img/hd/Home/V16/Home/continue-choose.png",
                click: function () {
                    H('pop')
                    clearInterval(timer)
                    cutDown = 5
    ...
    onClickRecommendPosition: function (btn) {
        ...
        Page.clickedButton = btn;
        // 小窗视频（tab0 二号位）
        if (pos == 12 && RenderParam.carrierId != '10000051') {
            // 辽宁电信小窗位置为普通推荐位
        ...
        if (pos == 11 || pos == 13 || pos == 21 || pos == 24 || pos == 31 || pos == 33 || pos == 41 || pos == 43) {
            var itemData = btn.cBanner;
        } else if(entryItem !== null){
            var itemData = entryItem.item_data[index];
        } else if (pos == 12 && RenderParam.carrierId == '10000051') {
            // 跳转全屏播放页面
            Play.playHomePollVideo(btn);
            return;
        }
        ...
        //ylp20210122
        if (itemData == "")
            itemData = entryItem.item_data[index];
        // 统计推荐位点击事件
        ...
        switch (parseInt(itemData.entry_type)) {
            ...
            case HomeEntryType.HEALTH_JUMP_VIDEO:
            ...
            case HomeEntryType.ENTRY_APP:
                PageJump.jumpThirdPartySP("000051")
                break;
            case HomeEntryType.ONE_CLICK_INQUIRY:
                // 目前版本UI暂未应用，如需添加一键问医功能，参照目前是用版本
                break;
        }
    }
};
...
var PageJump = {
    ...
    jumpVideoVisitHome: function (btn) {
        ...
        LMEPG.Intent.jump(objDoctorP2P, null);
    },
...
var Play = {
    ...
    startPollPlay: function () {
        ...
        //如果配置了活动小窗就不播放了。
        var entryItem = Page.getRecommendDataByPosition(12);
    ...
    onPlayEvent: function () {
        ...
        try {
            ...
            self.currPollVideoId++;
            self.currPollVideoId = self.currPollVideoId % videoCount;
            LMEPG.Log.info("onPlayEvent 获取播放视频序号 carrierId : " + LMSmallPlayer.carrierId +
                ",self.currPollVideoId:"+self.currPollVideoId+" video count:"+RenderParam.homePollVideoList.count+
            " video url:"+RenderParam.homePollVideoList.list[self.currPollVideoId].videoUrl);
            self.startPollPlay();
        } catch (e) {
...
(function (w) {
    ...
    Banner.prototype.banner = function () {
        console.log("banner>>>" + RenderParam.fsUrl + JSON.parse(this.data[0].img_url).normal)
        if (this.data.length > 1) {
            var that = this;
            this.timer = setInterval(function () {
                that.index < that.data.length ? that.index++ : that.index = 1;
                G(that.id).src = RenderParam.fsUrl + JSON.parse(that.data[that.index - 1].img_url).normal;
                ...
                LMEPG.BM.getButtonById(that.id).cBanner = that.data[that.index - 1];
...
var onBack = htmlBack = function () {
    ...
    switch (true) {
        ...
        case currentNavID != 'tab-0':
            var isBack = LMEPG.Func.getLocationString('isBack') || 0;
            if (isBack == 1) {
                LMEPG.Intent.back();
                return;
            }
            ...
            // 自定义链接返回局方大厅
            var backEpg = LMEPG.Func.getLocationString('backEpg') || 0;
            if (backEpg == 1) {
                LMEPG.Intent.back("IPTVPortal");
                return
            }
