...
var Page = {
    ...
    jumpInquiryDoctorList: function () {
        ...
        if (RenderParam.accessInquiryInfo.isAccessInquiry == 0
            ...
            var intentName;
            if(RenderParam.carrierId === '220095' || RenderParam.carrierId === '220094') {
                intentName = 'doctorIndex';
            } else {
                intentName = 'doctorList';
            }
            objDoctorP2P = LMEPG.Intent.createIntent(intentName);
            objDoctorP2P.setParam('userId', RenderParam.userId);
     ...
     * 跳转 - 健康检测IMEI号输入页面
     */
    jumpHealthTestIMEI: function () {
        var objCurrent = Page.getCurrentPage();
        var objHomeTab;
        if(RenderParam.carrierId === "650092" || RenderParam.carrierId === '220095'){
            objHomeTab = LMEPG.Intent.createIntent('testIndex');
        }else {
            objHomeTab = LMEPG.Intent.createIntent('healthTestIMEIInput');
        }
        LMEPG.Intent.jump(objHomeTab, objCurrent);
    },
     ...
     * 新疆电信 在线购买商城
     */
    jumpPayOnline: function () {
        var objHome = Page.getCurrentPage();

        var objEpidemic = LMEPG.Intent.createIntent('payOnline');
        objEpidemic.setParam("userId", RenderParam.userId);
        LMEPG.Intent.jump(objEpidemic, objHome);
    },
    ...
    jumpAbout: function () {
        ...
        LMEPG.Intent.jump(objAbout, objHome);
    },
    ...
    jumpModule: function (data) {
        switch (data.entryType) {
            ...
            case '14':
                // 预约挂号
                LMEPG.Intent.jump(LMEPG.Intent.createIntent('appointment'), Page.getCurrentPage());
                break;
            case '22':
                // 具体地址跳转
...
var Play = {
    ...
    startPollPlay: function () {
        ...
        switch (RenderParam.carrierId) {
            ...
            case '000051':
                ...
                break;
            case '220094'://吉林广电
                this.play220094(videoUrl);
                break;
            case '220095'://吉林广电电信新媒体
                this.play220095(videoUrl);
                break;
            case '340092':
    ...
    /**
     * 吉林广电小窗播放
     * @param videoId 注入局方的视频code
     */
    play220094: function (videoId) {
        setTimeout(function () {
            try {
                if (typeof window.top.jk39 !== 'undefined' && window.top.jk39 !== null && window.top.jk39.fetchPlayData !== undefined) {
                    window.top.jk39.fetchPlayData({
                            ...
                            callback: function (playUrl, name) {
                                LMEPG.Log.info('[home.js][V8]--->getPlayUrl220094--->callback: [' + videoId + ']-->[' + playUrl + ' , ' + name + ']');
                                var playPosition = [parseInt(435), parseInt(145), parseInt(550), parseInt(302)];
                                LMEPG.mp.initPlayerMode1();
                                LMEPG.mp.playOfSmallscreen(playUrl, playPosition[0], playPosition[1], playPosition[2], playPosition[3]);
                            }
                        }
                    );
                } else {
                    LMEPG.Log.info('[home.js][V8]--->getPlayUrl220094--->No "window.top.jk39.fetchPlayData" function!');
                }
            } catch (e) { //保护兼容
                LMEPG.Log.error('[home.js][V8]--->play220094--->Exception: ' + e.toString());
            ...
            try {
                if (typeof window.top.bestv !== 'undefined' && window.top.bestv !== null && window.top.bestv.fetchPlayData !== undefined) {
                    window.top.bestv.fetchPlayData({
                            ...
                            categoryId: '', //栏目ID，可选
                            callback: function (playUrl, name) {
                                LMEPG.Log.info('[home.js][V8]--->getPlayUrl220095--->callback: [' + videoId + ']-->[' + playUrl + ' , ' + name + ']');
                                var playPosition = [parseInt(435), parseInt(145), parseInt(550), parseInt(302)];
                                LMEPG.mp.initPlayerMode1();
                                LMEPG.mp.playOfSmallscreen(playUrl, playPosition[0], playPosition[1], playPosition[2], playPosition[3]);
                            }
                    ...
                    LMEPG.Log.info('[home.js][V8]--->getPlayUrl220095--->No "window.top.bestv.fetchPlayData" function!');
                }
            } catch (e) { //保护兼容
                LMEPG.Log.error('[home.js][V8]--->play220095--->Exception: ' + e.toString());
                LMEPG.UI.showToast('小窗获取播放地址异常！');
            }
        }, 500);
    },
    ...
    play650092: function (videoUrl) {
        setTimeout(function () {
            ...
            if (LMEPG.Func.isEmpty(stbDomainUrl)) {
                return;
            }
            ...
            LMEPG.mp.initPlayerByBind();
            if (RenderParam.carrierId == '650092' && (stbModel == 'B860A' || stbModel == 'B860AN-T2')) {
                setTimeout(function () {
...
var Home = {
    ...
    Banner: {
        ...
        transModuleData: function (data) {
            ...
            if (data.entry_type == '20') {
                ...
                configInfo.freeSeconds = tempInnerParametersObj.free_seconds;
        ...
        updateIndicator: function () {
            var indicatorImage = g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/icon_indicator_point.png';
            var indicatorSelectedImage = g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/icon_indicator_point_show.png';
            // 设置指示器位置
    ...
    /**
     * 通过当前导航栏索引号得到其左右相邻的tab路由名称。由 Home.onNavBeforeMoveChange 调用！
     * @param tabIndex 导航栏索引号（0-4）
     * @return {{left: {routerName: string, navId: string}, right: {routerName: string, navId: string}}}
     * @author Songhui 2019-6-4 简单动态获取当前tab导航栏相邻页面的信息，用于tab_nav导航栏焦点切换即页面切换！
     */
    getNearByNav: function (tabIndex) {
        var minIndex = 0, maxIndex = 4; //顶部导航栏id取值范围
        var leftRouterName = '', rightRoutName = ''; //切换到左/右相邻的tab页面路由名
        var leftNavId = '', rightNavId = ''; //切换到左/右相邻的tab页面导航栏id
        if (tabIndex >= minIndex && tabIndex <= maxIndex) {
            leftRouterName = tabIndex === 1 ? 'home' : tabIndex <= minIndex ? 'homeTab' + maxIndex : 'homeTab' + (tabIndex - 1);
            rightRoutName = tabIndex === maxIndex ? 'home' : 'homeTab' + (tabIndex + 1);
            leftNavId = 'nav_' + (tabIndex === minIndex ? maxIndex : tabIndex - 1);
            rightNavId = 'nav_' + (tabIndex === maxIndex ? minIndex : tabIndex + 1);
        } else {
            console.error('getNearByNav(' + tabIndex + ')--->Out of bound!');
        }
        return {
            left: {
                routerName: leftRouterName,
                navId: leftNavId
            },
            right: {
                routerName: rightRoutName,
                navId: rightNavId
            }
        };
    },
    ...
    onNavBeforeMoveChange: function (dir, current) {
        // 导航栏焦点移动，即切换对应页面。（TODO 应吉林广电要求更改）
        if (RenderParam.carrierId === '220094' || RenderParam.carrierId === '220095') {
            if (dir === 'left' || dir === 'right') {
                try {
                    var jumpTabConfig = eval('Home.getNearByNav(' + current.cNavId + ').' + dir);
                    Page.jumpHomeTab(jumpTabConfig.routerName, jumpTabConfig.navId);
                    return false;
                } catch (e) {
                }
            }
    ...
    onMenuClick: function (btn) {
        switch (btn.id) {
            case 'menu_border_0':
                // 成为vip
                Page.jumpBuyVipWithNormal('HomeMenu');
                break;
            case 'menu_border_1':
    ...
    initMenu: function () {
        buttons.push({
            ...
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_menu_border.png',
    ...
    initRecommend: function () {
        buttons.push({
            ...
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_1.png',
        ...
        buttons.push({
            ...
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_2.png',
            click: Home.onRecommendClick,
        ...
        buttons.push({
            ...
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_3.png',
            click: Home.onRecommendClick,
            focusChange: Home.onRecommendFocusChange,
            beforeMoveChange: Home.onRecommendBeforMoveChange,
            moveChange: LMEPG.emptyFunc,
        ...
        buttons.push({
            ...
            name: '推荐位4',
            type: 'img',
            nextFocusLeft: 'menu_border_3',
            nextFocusRight: 'recommend_border_5',
            nextFocusUp: 'recommend_border_1',
            nextFocusDown: '',
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_4.png',
            click: Home.onRecommendClick,
            focusChange: Home.onRecommendFocusChange,
            beforeMoveChange: Home.onRecommendBeforMoveChange,
            moveChange: LMEPG.emptyFunc,
        ...
        buttons.push({
            ...
            nextFocusDown: '',
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_5.png',
            click: Home.onRecommendClick,
            focusChange: Home.onRecommendFocusChange,
            beforeMoveChange: Home.onRecommendBeforMoveChange,
            moveChange: LMEPG.emptyFunc,
        ...
        buttons.push({
            ...
            nextFocusDown: '',
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + RenderParam.classifyId + '_6.png',
            click: Home.onRecommendClick,
            focusChange: Home.onRecommendFocusChange,
            beforeMoveChange: Home.onRecommendBeforMoveChange,
            moveChange: LMEPG.emptyFunc,
        ...
        buttons.push({
            ...
            nextFocusDown: '',
            backgroundImage: g_appRootPath + '/Public/img/Common/spacer.gif',
            focusImage: g_appRootPath + '/Public/img/' + RenderParam.platformType + '/Home/V8/bg_home_recommend_border_' + +RenderParam.classifyId + '_7.png',
            click: Home.onRecommendClick,
            focusChange: Home.onRecommendFocusChange,
            beforeMoveChange: Home.onRecommendBeforMoveChange,
            moveChange: LMEPG.emptyFunc,
    ...
    init: function () {
        var stbModel = LMEPG.STBUtil.getSTBModel();
        if (stbModel == 'B860A' || stbModel == 'B860AN-T2') {
        ...
        //启动字幕滚动
        LMEPG.UI.Marquee.start('scroll_message', 7, 2, 50, 'left', 'scroll');
