        ...
        if (RenderParam.homeConfigInfo.result == 0) {
            for (var i = 0; i < RenderParam.navConfig.length; i++) {
                ...
                for (var j = 0; j < entryList.length; j++) {
                    ...
                    if (entryList[j].position.substr(0, 1) == (i + 1 + '')) {
                        // 匹配到导航栏
...
var Play = {
    ...
    startPollPlay: function () {
        ...
        switch (RenderParam.carrierId) {
            ...
            case '640092':
                ...
                break;
            case '520092':
                this.play520092(videoUrl, RenderParam.playPosition);
                break;
            default:
                this.play630092(videoUrl, RenderParam.playPosition);
                break;
    ...
    playHomePollVideo: function () {
        ...
        var videoObj = JSON.parse(data.ftp_url);
        var videoUrl = RenderParam.platformType == 'hd' ? videoObj.gq_ftp_url : videoObj.bq_ftp_url;

        var showStatus = "";
        if (typeof (data.show_status) !== "undefined" && data.show_status != "") {
            showStatus = data.show_status;
        } else if (typeof (data.showStatus) !== "undefined" && data.showStatus != "") {
    ...
    getCurrentPollVideoUrl: function () {
        var videoUrl = '';
        var count = GConfigData.navConfig[0].recommendList[1].length;
        if (Play.currPollVideoId >= 0 && Play.currPollVideoId < count) {
            var videoObj = JSON.parse(GConfigData.navConfig[0].recommendList[1][Play.currPollVideoId].ftp_url);
     ...
     * @param videoUrl
     */
    play520092: function (videoUrl) {
        setTimeout(function () {
            LMEPG.mp.initPlayer();
            var playPosition = [parseInt(0), parseInt(0), parseInt(0), parseInt(0)]; // left,top,width,height
            if (RenderParam.platformType == 'hd') {
                playPosition = [parseInt(74), parseInt(112), parseInt(532), parseInt(300)];
            } else {
                playPosition = [parseInt(175), parseInt(115), parseInt(282), parseInt(160)];
            }
            // 开始小窗播放
            LMEPG.mp.playOfSmallscreen(videoUrl, playPosition[0], playPosition[1], playPosition[2], playPosition[3]);
        }, 500);
    },
    ...
    play640092: function (videoUrl, playPosition) {
        ...
        setTimeout(function () {
            ...
            LMEPG.Log.error('thirdPlayerUrl playUrl : ' + factUrl);
    ...
    onPlayEvent: function (keyCode) {
        if (LMEPG.mp.isEnd(keyCode) || LMEPG.mp.isError(keyCode)) {
            var videoCount = GConfigData.navConfig[0].recommendList[1].count;
...
var Home = {
    ...
    setBodyBg: function (id) {
        var TP = RenderParam;
        var bgImg = '';
        if ((LMEPG.Func.isExist(RenderParam.themeImage) && RenderParam.themeImage !== "") && (LMEPG.Func.isExist(RenderParam.themeNoPlay) && RenderParam.themeNoPlay !== "")) {
            bgImg = RenderParam.fsUrl + RenderParam.themeNoPlay;
    ...
    initStaticButton: function () {
        ...
        buttons.push({
                ...
                nextFocusUp: '',
                nextFocusDown: 'r-img-wrap-1',
                backgroundImage: g_appRootPath + '/Public/img/hd/Home/V11/payInfo.png',
                focusImage: g_appRootPath + '/Public/img/hd/Home/V11/payInfo-f.png',
                click: Page.jumpPayInfo,
                focusChange: '',
                beforeMoveChange: Home.staticVideoPosition,
                moveChange: '',
                cIdx: ''
    ...
    loadContainer: function (index, classType) {
        ...
        htm += '<div class=\'load-content load-content-' + classType + '\'>';
    ...
    createBtns: function (index, sid) {
        ...
        while (i < maxLength) {
            ...
            LMEPG.ButtonManager.addButtons({
                ...
                click: Home.onClickRecommendPosition,
                focusChange: this.loadBtnOnFocusChange,
                beforeMoveChange: this.downMoveLoaderModules,
                moveChange: '',
                cSid: sid,      // 样式类别
                cNavId: index,
                cPosition: 0,
    ...
    scrollDistanceTop: function (dis) {
        var scrollTopDom = G('container');
        console.log('scrollDistanceTop ---> before distance: ' + scrollTopDom.scrollTop);
        scrollTopDom.scrollTop = dis === 0 ? 0 : scrollTopDom.scrollTop + dis;
        console.log('scrollDistanceTop ---> after distance: ' + scrollTopDom.scrollTop);
        if (scrollTopDom.scrollTop > 0) {
            Home.backFocus = false;
            Home.underBottomTips('<img src="' + g_appRootPath + '/Public/img/hd/Home/V11/bottom-tips.png" alt=""/>');
            Home.toggleBgImage(1);
            if (Play.isPlayStarted()) {
                LMEPG.mp.pause();  //暂停播放
            }
        } else {
            Home.toggleBgImage(0);
            Home.underBottomTips('');
            if (Play.isPlayStarted()) {
                LMEPG.mp.resume(); // 恢复播放
            } else {
                Play.startPollPlay();
            }
        }
        ...
        switch (btn.id) {
            ...
            default:
                ...
                switch (parseInt(data[btn.cIdx].entry_type)) {
                    ...
                    case HomeEntryType.MY_FIMILY_HOME_45:
                        Page.jumpMyFamilyUI();
                        //我的家
                        break;
...
var Page = {
    ...
    jumpHealthMeasure: function () {
        ...
        var objDst = LMEPG.Intent.createIntent(RenderParam.carrierId == '520092'? 'testIndex' : 'healthTestImeiInput');
