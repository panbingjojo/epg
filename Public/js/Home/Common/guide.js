var url = g_appRootPath + '/Public/img/hd/Home/V10/';
var Guide = {
    mainButton: null,
    buttons: [],
    stepNum: 0,
    ...
    open: function (step, type) {
        if (!RenderParam.firstHandle) {
            console.log("is not firstHandle");
            return;
        }
    ...
    initHtml: function () {
        var html = "";
        html += '<img id="guide-bg" class="hold_bg" src="' + g_appRootPath + url + Guide.type + '_1.png"/>';
     ...
     * @private
     */
    _saveFirstHandle: function () {
        if (RenderParam.firstHandleName === undefined || RenderParam.firstHandleName === null) {
            console.log("_saveFirstHandle::firstHandleName is not define");
            return;
        }
        var $reqData = {
            "firstHandleName": RenderParam.firstHandleName
        };
        LMEPG.ajax.postAPI("Guide/setIsFirstHandle", $reqData, function (rsp) {
            console.log("_saveFirstHandle::" + rsp.toString());
        });
    }
