...
var renderPage = {
    ...
    navHtml: function () {
        ...
        for (var index = 0; index < 4; index++) {
                if(!navImages[index]) continue;
                htm += '<img id=nav-'+ index +' src='+  (navImages[index]).backgroundImage +'>';
        }

        this.$.nav.innerHTML = htm;
    },

    dispatchTabName: function(){
        var tabNames = [];
        switch (RenderParam.carrierId) {
            case '370092':
                tabNames = ['宝贝指南','女性宝典','老年健康','健康百科'];
                break;
            default:
                tabNames = ['老年健康','女性宝典','宝贝指南','健康百科'];
                break;
        }
        return tabNames;
    },
