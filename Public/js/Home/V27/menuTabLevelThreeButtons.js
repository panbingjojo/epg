 ...
 *  导航栏对应图片
 */
var navImages = (function () {
    var navImageArr = [];
    switch (RenderParam.carrierId) {
        case '370002':
            for (var i = 0; i < 4; i++) {
                navImageArr.push({
                    backgroundImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/V" + RenderParam.carrierId + "/nav" + i + ".png",
                    focusImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/V" + RenderParam.carrierId + "/nav" + i + "_f.png",
                    selectedImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/V" + RenderParam.carrierId + "/nav" + i + "_s.png"
                })
            }
            break;
        default:
            for (var index = 0; index < 4; index++) {
                navImageArr.push({
                    backgroundImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/nav" + index + ".png",
                    focusImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/nav" + index + "_f.png",
                    selectedImage: ROOT + "/Public/img/hd/Home/V27/menu/tab" + RenderParam.homeTabIndex + "/nav" + index + "_s.png"
                })
            }
            break;
    }
...
var buttons = [{
     ...
     */
    {
        ...
        backgroundImage: (navImages[0]).backgroundImage,
        focusImage: (navImages[0]).focusImage,
        selectedImage: (navImages[0]).selectedImage,
        click: clickSelectedFocus,
        focusChange: judgeSelectedFocus,
        beforeMoveChange: keepLeaveNavId,
        moveChange: "",
        cIdx: 0,
    }, {
        id: 'nav-1',
        ...
        nextFocusUp: 'search',
        nextFocusDown: 'video-link-0',
        backgroundImage: (navImages[1]).backgroundImage,
        focusImage: (navImages[1]).focusImage,
        selectedImage: (navImages[1]).selectedImage,
        click: clickSelectedFocus,
        focusChange: judgeSelectedFocus,
        beforeMoveChange: keepLeaveNavId,
        moveChange: "",
        ...
        nextFocusUp: 'search',
        nextFocusDown: 'video-link-0',
        backgroundImage: (navImages[2]).backgroundImage,
        focusImage: (navImages[2]).focusImage,
        selectedImage: (navImages[2]).selectedImage,
        click: clickSelectedFocus,
        focusChange: judgeSelectedFocus,
        beforeMoveChange: keepLeaveNavId,
        moveChange: "",
        ...
        nextFocusUp: 'search',
        nextFocusDown: 'video-link-0',
        backgroundImage: navImages[3] ? (navImages[3]).backgroundImage :"",
        focusImage: navImages[3] ?  (navImages[3]).focusImage : "",
        selectedImage: navImages[3] ?  (navImages[3]).selectedImage :"",
        click: clickSelectedFocus,
        focusChange: judgeSelectedFocus,
        beforeMoveChange: keepLeaveNavId,
        moveChange: "",
...
function clickSelectedFocus(btn) {
    ...
    for (var i = 0; i < currentIdChildList.length; i++) {
        currentIdChildList[i].src = navImages[i].backgroundImage;
    }
...
function judgeSelectedFocus(btn, focus) {
    if (focus) {
        ...
        G("nav-" + currentTab).src = navImages[currentTab].selectedImage;
    }
}

function keepLeaveNavId(dir,btn) {
    if (dir === "up" || dir === "down") renderPage.keepNavId = btn.id;
}
...
function onFocusChangeBg(btn, focus) {
    ...
    var elementP = currentFocusId.getElementsByTagName("p")[0];
    var text = elementP.innerText;
    if (focus) {
        elementImg.className = "active";
        text.length > 11 && (elementP.innerHTML = "<marquee>" + text + "</marquee>");
    } else {
        elementImg.className = "";
        text.length > 11 && (elementP.innerHTML = text);
    }
}
