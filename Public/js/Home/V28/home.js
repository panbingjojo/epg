...
var Page = {
    ...
    init: function () {
        ...
        G(this.tabId + "-scroll").style.left = RenderParam.tabScroll + "px";
        this.stopSmallVideo();
    },
    ...
    stopSmallVideo: function () {
        if(!LMEPG.Func.isEmpty(this.focusId) && this.focusId.indexOf('tab0-recommended-3-list-') >= 0){
            isTab0TurnPages = true;
            reBuildSmallVideo(2);
            setBodyBackgroundImage(2);
        }
    },
     ...
     * 点击Tab2的专辑
     */
    onClickTab5Album: function (btn) {
        var album = RenderParam.albumList.list[btn.cIndex];
        if (!album) {
            LMEPG.UI.showToast('未配置该专辑哦 ~');
            return;
        }
        PageJump.jumpAlbumPage(album.alias_name, btn);
    },
    ...
    onClickRecommendPosition: function (btn) {
        ...
        if(pos == 11 && isSmallVideo){
            Play.playHomePollVideo(); // 跳转全屏播放页面
            return;
        }
        var index = 0; // 一个推荐位可能有多条数据，一般为1条。多条的特殊处理，index表示数组下标
        if (btn.cBanner != undefined) {
            var itemData = entryItem.item_data[btn.cBanner];
        } else {
...
var Play = {
    ...
    startPollPlay: function () {
        ...
        for(var i = 0; i < list.length; i++) {
            if(list[i].position == 11){
                return;
            }
        }
        isSmallVideo = true;
        G('tab0-recommended-1').src = "";
        // 避免多次调用
        ...
        if (globalPath.platformType == 'hd') {
            switch (globalPath.stbModel) {
                ...
                case 'E900V21D': // 区分盒子传递位置参数
                    param.position = {top: 241, left: 100, width: 420, height: 230}; // 按比例420/235，不按比例有黑边
                    break;
                default:
                    param.position = {top: 241, left: 100, width: 420, height: 230};
                    break;
            }
        } else {
            //param.position = {top: 239, left: 100, width: 420, height: 230};// 中国联通标清
        }
...
var onBack = htmlBack = function () {
    ...
    switch (true) {
        ...
        case currentNavID != 'tab-0':
            ...
            if (LMEPG.BM.getCurrentButton().id.substring(0, 4) != "tab-") {
                LMEPG.BM.getButtonById(currentNavID).nextFocusDown = LMEPG.BM.getCurrentButton().id;
            }
...
function getPollTimes() {
    if(!isStartSmallVideo){
        return;
    }
    var mpTimes = LMEPG.mp.getTimes();
    LMEPG.Log.debug('getmpTimes position: ' + mpTimes.position+"--duration:"+mpTimes.duration);
    if(mpTimes.position > 0 && mpTimes.position+1 == mpTimes.duration ){
        Play.startPollPlay();
    }
    setTimeout(function () {
        getPollTimes();
