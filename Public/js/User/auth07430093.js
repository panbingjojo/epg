LMEPG.AuthUser.getEPGParams = function () {
    ...
    if (typeof (window.starcorExt) === "undefined"/*无实际环境*/ || typeof (mangoTV) === "undefined"/*未引入js*/) {
        ...
        var tvInfo = mangoTV.get_all_unicom_info();
        EPGParams = {
            stbModel: tvInfo.model,                  // 盒子型号，包括厂商和具体系列
            stbMac: tvInfo.deviceMac,                // 盒子MAC地址
            epgDomain: LMEPG.STBUtil.getEPGDomain(), // 局方大厅的地址，用于应用程序返回局方大厅
            stbId: tvInfo.stbid,                     // 盒子的设备ID
            userToken: tvInfo.userToken,             // 用户当前的token值
